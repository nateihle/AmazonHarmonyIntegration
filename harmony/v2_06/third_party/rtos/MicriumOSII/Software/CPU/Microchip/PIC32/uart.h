...
*************************************************************/
#define UART_ERR_INT    			    (1 << 6)  	/* Error interrupt     */
#define UART_RX_INT      			    (1 << 7)  	/* Receive interrupt   */
#define UART_TX_INT      			    (1 << 8)  	/* Transmit interrupt  */
...
#define ConfigIntUART1(config)	( mU1ClearAllIntFlags(), SetPriorityIntU1(config), \
	SetSubPriorityIntU1(config), mU1IntEnable((((config)>> 6) & 7)) )
#define EnIntUART1(config)      (mU1IntEnable((((config)>> 6) & 7)) )
#define DisIntUART1(config)     (mU1IntDisable((((config)>> 6) & 7)) )
#endif

#if defined(_UART2)
#define ConfigIntUART2(config) ( mU2ClearAllIntFlags(), SetPriorityIntU2(config), \
	SetSubPriorityIntU2(config), mU2IntEnable((((config)>> 6) & 7)) )
#define EnIntUART2(config)      (mU2IntEnable((((config)>> 6) & 7)) )
#define DisIntUART2(config)     (mU2IntDisable((((config)>> 6) & 7)) )
#endif
 ...
 * Do not use these defines for Daytona native applications.
 *****************************************************************************/
	...
	// On Daytona, there are no separate priorities for RX and TX interrupt
