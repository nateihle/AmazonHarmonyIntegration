...
typedef  CPU_INT32U  CPU_SR;                                /* Defines CPU status reg size (SR = 32 bits).              */

#define  CPU_CRITICAL_ENTER()           {cpu_sr |= mCTGetIntEnable() << 3; mCTIntEnable(0); cpu_sr = INTDisableInterrupts();}
#define  CPU_CRITICAL_EXIT()            {INTRestoreInterrupts(cpu_sr); mCTIntEnable((cpu_sr >> 3) & 0x1);}
