...
// KEEP THE FOLLOWING CHANGES FOR THE USART INTERRUPT DRIVER:
extern APP_DATA appData;

extern uint8_t APP_MAKE_BUFFER_DMA_READY uartReceivedData[APP_READ_BUFFER_SIZE];
extern uint8_t APP_MAKE_BUFFER_DMA_READY uartTransmitData[APP_READ_BUFFER_SIZE];

extern const DRV_USART_INIT drvUsart0InitData;      // contains the baud member set in MHC

extern uint16_t uartReceiveHead, uartReceiveTail;
extern uint16_t uartTransmistHead, uartTransmitTail; 
...
void __ISR(_UART_2_VECTOR, ipl1AUTO) _IntHandlerDrvUsartInstance0(void)
{
    ...
    {
        while(DRV_USART_ReceiverBufferIsEmpty(appData.usartHandle)==0)
        {
            appData.uartReceivedData[uartReceiveHead++] = DRV_USART_ReadByte(appData.usartHandle);
            if (uartReceiveHead > (APP_READ_BUFFER_SIZE-1))
            {
                uartReceiveHead = 0;
            }
        }        
