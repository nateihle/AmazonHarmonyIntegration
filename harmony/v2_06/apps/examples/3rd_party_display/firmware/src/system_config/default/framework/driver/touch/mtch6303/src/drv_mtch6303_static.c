                                        ...
                                        const SYS_MODULE_INIT * const init )
{
 ...
 // CUSTOM CODE - DO NOT DELETE
    //Reset the FocalTech touch controller
    CTP_RSTStateSet(0);
    _DelayMS(5);
    CTP_RSTStateSet(1);
// END OF CUSTOM CODE
...
bool _DRV_TOUCH_MTCH6303_ClientBufferQueueObjectsRemove( void )
{
    ...
    interruptWasEnabled = SYS_INT_SourceDisable(INT_SOURCE_EXTERNAL_0);
...
// END OF CUSTOM CODE
            ...
            {
                //Translate the touch data to X/Y coordinates
                hDriver->mostRecentTouchX = (int16_t)DRV_TOUCH_MTCH6303_TouchInputMap(hDriver->touchData.touch[0].x, 800);
                hDriver->mostRecentTouchY = (int16_t)DRV_TOUCH_MTCH6303_TouchInputMap(hDriver->touchData.touch[0].y, 480);
            }
            else
            {
                hDriver->mostRecentTouchX = -1;
