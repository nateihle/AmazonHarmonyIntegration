...
void APP_SQI_Initialize(void)
{
    ...
    PLIB_SQI_ClockDividerSet(APP_SQI_ID_0, CLK_DIV_1);
...
void APP_SQI_Flash_Erase (void)
{
    ...
    PLIB_SQI_ControlWordSet(APP_SQI_ID_0, 1, APP_SQI_CS_1, APP_SQI_LANE_QUAD, APP_SQI_CMD_TRANSMIT, 1);     /* GBLKUP */

    /* Send necessary commands to un-protect the flash blocks*/
    PLIB_SQI_TransmitData(APP_SQI_ID_0, (FLASH_UNPROTECT_GLOBAL << 24) | (FLASH_WEN << 16) | (FLASH_NOP << 8) | FLASH_NOP);
...
SQI_STATUS APP_SQI_Flash_ID_Check (void)
{
    ...
    PLIB_SQI_ControlWordSet(APP_SQI_ID_0,1,APP_SQI_CS_1,APP_SQI_LANE_QUAD,APP_SQI_CMD_TRANSMIT,1);          /* NOP */
    PLIB_SQI_ControlWordSet(APP_SQI_ID_0,0,APP_SQI_CS_1,APP_SQI_LANE_QUAD,APP_SQI_CMD_TRANSMIT,2);          /* QJID & DUMMY */   
    ...
    PLIB_SQI_TransmitData(APP_SQI_ID_0, (FLASH_NOP << 24) | (FLASH_QJID << 16) | (FLASH_NOP << 8) | (FLASH_EQIO));
...
void APP_SQI_PIO_PageWrite (uint32_t flashAddress, uint8_t* writeBuffer)
{
    ...
    while (!(PLIB_SQI_InterruptFlagGet(APP_SQI_ID_0, APP_SQI_TXTHR)));
    ...
    _APP_SQI_StartCoreTimer(0);
    _APP_SQI_CoreTimer_Delay(200000);
}
...
void APP_SQI_PIO_PageRead(uint32_t flashAddress, uint32_t* readBuffer)
{
    ...
    PLIB_SQI_ControlWordSet(APP_SQI_ID_0,0,APP_SQI_CS_1,APP_SQI_LANE_QUAD,APP_SQI_CMD_TRANSMIT,3);
    ...
    PLIB_SQI_TransmitData(APP_SQI_ID_0, sqiCmdBuffer[2] << 24 | sqiCmdBuffer[1] << 16 | (sqiCmdBuffer[0] << 8) | FLASH_NOP);

    /* Write address and dummy bytes {DUMMY, ADDRESS[3:0]} */
    PLIB_SQI_TransmitData(APP_SQI_ID_0, (0x000000 << 8) | sqiCmdBuffer[3]);

    /* Setup control word to read 256 bytes */
    PLIB_SQI_ControlWordSet(APP_SQI_ID_0,1,APP_SQI_CS_1,APP_SQI_LANE_QUAD,APP_SQI_CMD_RECEIVE,FLASH_PAGE_SIZE);

    /* Setup receive buffer threshold */
    _APP_SQI_BufferThrSet(0, 0, 16);

    for (readLoop=0; readLoop < 16; readLoop++)
    {
        while ((PLIB_SQI_RxBufferThresholdGet(APP_SQI_ID_0)& 0xFF) != 16);
...
uint8_t APP_SQI_FlashReadStatus(void)
{
    ...
    /* Transmit RDSR */
    PLIB_SQI_TransmitData(APP_SQI_ID_0, (FLASH_NOP << 24) | (FLASH_RDSR << 16) | (FLASH_NOP << 8) | FLASH_NOP);
