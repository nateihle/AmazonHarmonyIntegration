...
void APP_SQI_PIO_PageWrite (uint32_t flashAddress, uint8_t* writeBuffer)
{
    ...
    _APP_SQI_StartCoreTimer(0);
    _APP_SQI_CoreTimer_Delay(18000);
}

void APP_SQI_XIP_Read (uint32_t flashAddress, uint32_t readSize, uint8_t* readBuffer)
{
    uint32_t readLoop;
    uint32_t * sqiXIPAddr;
    ...
    PLIB_SQI_XIPControlWord1Set(APP_SQI_ID_0,APP_DUMMY_BYTES_3,APP_ADDR_BYTES_3,
                                FLASH_FAST_READ,APP_SQI_LANE_QUAD,
                                APP_SQI_LANE_QUAD,APP_SQI_LANE_QUAD,
                                APP_SQI_LANE_QUAD,APP_SQI_LANE_QUAD);

    // Configure XIP Control 2 SFR (chip select 1)
    PLIB_SQI_XIPControlWord2Set(APP_SQI_ID_0,APP_SQI_CS_1,APP_MODE_BYTES_0,0);

    // Set to XIP mode
    PLIB_SQI_TransferModeSet(APP_SQI_ID_0, APP_SQI_XFER_MODE_XIP);
