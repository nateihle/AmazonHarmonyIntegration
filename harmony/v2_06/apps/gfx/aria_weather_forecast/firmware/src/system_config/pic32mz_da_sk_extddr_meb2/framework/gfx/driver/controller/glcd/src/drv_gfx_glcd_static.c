...
void layerSwapped(GFX_Layer* layer)
{
    if(layer->id == GFX_ActiveContext()->layer.layers[0].id)
    {
        ...
        PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 0, (uint32_t)drvLayer[0].baseaddr[layer->buffer_read_idx]);
    }
    if(layer->id == GFX_ActiveContext()->layer.layers[1].id)
    {
        ...
        PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 1, (uint32_t)drvLayer[1].baseaddr[layer->buffer_read_idx]);
    }
    if(layer->id == GFX_ActiveContext()->layer.layers[2].id)
    {
        ...
        PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 2, (uint32_t)drvLayer[2].baseaddr[layer->buffer_read_idx]);
    }
}
...
static GFX_Result glcdInitialize(GFX_Context* context)
{
    ...
    PLIB_GLCD_ClockDividerSet(GLCD_ID_0, 10);
	...
	drvLayer[0].baseaddr[0] = (uint32_t*)GFX_GLCD_LAYER0_BASEADDR;
	drvLayer[0].baseaddr[1] = (uint32_t*)GFX_GLCD_LAYER0_DBL_BASEADDR;
	drvLayer[1].baseaddr[0] = (uint32_t*)GFX_GLCD_LAYER1_BASEADDR;
	drvLayer[1].baseaddr[1] = (uint32_t*)GFX_GLCD_LAYER1_DBL_BASEADDR;
	drvLayer[2].baseaddr[0] = (uint32_t*)GFX_GLCD_LAYER2_BASEADDR;
	drvLayer[2].baseaddr[1] = (uint32_t*)GFX_GLCD_LAYER2_DBL_BASEADDR;
