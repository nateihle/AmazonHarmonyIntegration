...
void appTimerCallback ( uintptr_t context, uint32_t currTick )
{
    appData.timerTick++;
}
...
#ifndef NO_RADIAL_MENU_DEMO
        case APP_STATE_RADIAL_MENU_DEMO:
        {
            screenID = RadialMenuDemoScreen_ID;
            break;
        }
#endif        
        default:
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        {            
            ...
            {
                SYS_TMR_CallbackStop(appData.appTimer);
                appData.appTimer = SYS_TMR_CallbackPeriodic(APP_TICK_TIMER_PERIOD_MS,
                                                            (uintptr_t) NULL,
                                                            (SYS_TMR_CALLBACK) appTimerCallback);  
