...
*/
#define CAL_X_INSET    (((480)*(20>>1))/100)
...
*/
#define CAL_Y_INSET    (((272)*(20>>1))/100)
...
short xRawTouch[DRV_TOUCH_ADC_SAMPLE_POINTS] = { 0x0096,
                                                         0x0371,
                                                         0x036F,
                                                         0x008E };

short yRawTouch[DRV_TOUCH_ADC_SAMPLE_POINTS] = { 0x00C9,
                                                         0x00C3,
                                                         0x0351,
                                                         0x0350 };
...
short DRV_TOUCH_ADC_PositionDetect(void)
{
    ...
    switch ( pDrvInstance->state ) 
    {
        ...
        {            
            if ( !PLIB_ADCHS_AnalogInputDataIsReady(DRV_ADC_ID_1, ADCHS_AN14) )
            {
                break; 
            }
            if ( (uint16_t) 256 <
                (uint16_t) PLIB_ADCHS_AnalogInputResultGet(DRV_ADC_ID_1, ADCHS_AN14)>>2 )
            {
                adcX = -1;
                adcY = -1;
            } 
        ...
        {
            if ( !PLIB_ADCHS_AnalogInputDataIsReady(DRV_ADC_ID_1, ADCHS_AN21) )
            {
                break;
            }

            if ((uint16_t) 256 >
                (uint16_t) PLIB_ADCHS_AnalogInputResultGet(DRV_ADC_ID_1, ADCHS_AN21)>>2)
            {
                //y+
        ...
        case DRV_TOUCH_ADC_STATE_GET_X:
        {
            ...
            temp = (uint16_t) PLIB_ADCHS_AnalogInputResultGet(DRV_ADC_ID_1, ADCHS_AN21)>>2;
            if (temp != tempX) 
            {
                tempX = temp;
                pDrvInstance->state = DRV_TOUCH_ADC_STATE_RUN_X;
        ...
        case DRV_TOUCH_ADC_STATE_GET_Y:
        {
            ...
            temp = (uint16_t) PLIB_ADCHS_AnalogInputResultGet(DRV_ADC_ID_1, ADCHS_AN14)>>2;
...
void DRV_TOUCH_ADC_CoefficientSet(short xraw[], short yraw[])
{
    ...
    yPoint[2] = yPoint[3] = (272 - CAL_Y_INSET);
    xPoint[0] = xPoint[3] = CAL_X_INSET;
    xPoint[1] = xPoint[2] = (480 - CAL_X_INSET);
