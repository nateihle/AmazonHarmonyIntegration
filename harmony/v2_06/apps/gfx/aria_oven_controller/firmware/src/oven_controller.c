...
static SYS_TMR_HANDLE actionTimer;
...
extern uint16_t __attribute__((coherent, aligned(32))) frameBuffer[BUFFER_PER_LAYER][DISPLAY_WIDTH * DISPLAY_HEIGHT];
...
static void actionTimer_Callback ( uintptr_t context, uint32_t currTick )
{ 
    typedef enum
    {
        SMOKE1, SMOKE2, SMOKE3,
        SMOKE4, SMOKE5, SMOKE6
    } SMOKE_STATES;
    static SMOKE_STATES smokeState = SMOKE1;
    
    n2d_rectangle_t dest, src;
    int i;
    ...
    switch (appData.event)
    {
        case APP_EVENT_SHRINK_OVEN_LOGO:
        {
            ...
            {
                appData.event = APP_EVENT_SHOW_BUTTONS;
            }
            break;
        }
        
        case APP_EVENT_SHOW_BUTTONS:
        {
            // show fish button
            ...
            SYS_TMR_ObjectDelete(actionTimer);
        ...
        case APP_EVENT_ANIMATE_SMOKE_START:
        {
            switch (smokeState)
            {
                case SMOKE1:             // show smoke sprite 1
                    src.x = 90; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE2;
                    break;
                case SMOKE2: // show smoke sprite 2
                    src.x = 151; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE3;
                    break;
                case SMOKE3: // show smoke sprite 3
                    src.x = 212; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE4;
                    break;
                case SMOKE4: // show smoke sprite 4
                    src.x = 273; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE5;
                    break;
                case SMOKE5: // show smoke sprite 5
                    src.x = 333; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE6;
                    break;
                case SMOKE6: // show smoke sprite 6
                    src.x = 400; src.y = 93; src.width = 60; src.height = 100;
                    smokeState = SMOKE1;
                    break;
            } 
            _DelayMS(100);
            dest.x = 400; dest.y = 97; dest.width = 60; dest.height = 163;
            n2d_blit(&layer0, &dest, &stage, &src, N2D_BLEND_NONE); 
        ...
        {
            dest.x = 400; dest.y = 97; dest.width = 60; dest.height = 163;
            n2d_fill(&layer0, &dest, 0x00, N2D_BLEND_NONE); 
            appData.event = APP_EVENT_SHRINK_OVEN_LOGO;
            SYS_TMR_ObjectDelete(actionTimer);
            break;
        }        
...
void APP_AnimateControllerScreen()
{
    ...
    SYS_TMR_ObjectDelete(actionTimer);
    actionTimer = SYS_TMR_CallbackPeriodic(
                            20, 
                            1, 
...
{ 
    ...
    layer0.gpu = KVA_TO_PA(frameBuffer);
    layer0.memory = (void*)frameBuffer;
