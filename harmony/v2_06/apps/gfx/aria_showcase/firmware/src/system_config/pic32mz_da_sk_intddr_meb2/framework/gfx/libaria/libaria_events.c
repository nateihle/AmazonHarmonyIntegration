...
static SYS_TMR_HANDLE handleTimerFF;
static SYS_TMR_HANDLE handleTimerPlay;

static void Timer_Callback ( uintptr_t context, uint32_t currTick )
{
...
{
    // Show Screen (TouchTestScreen) - Show Screen - TouchTestScreen
    laContext_SetActiveScreen(TouchTestScreen_ID);
}
...
{
    // Custom Action
    APP_SendEvent(APP_EVENT_RETURN_TO_MAIN_MENU);
}
...
void FourthScreen_ShowEvent(laScreen* scr)
{
    ...
    snprintf(value, 4, "%d", sliderPercentage);
...
void SliderWidget1_ValueChangedEvent(laSliderWidget* sld)
{
    // ChangeValue
    char value[4] = {0};
    uint32_t image1Alpha;
    uint32_t sliderPercentage = laSliderWidget_GetSliderPercentage(sld);
    laContext* context = laContext_GetActive();
    
    snprintf(value, 4, "%d", sliderPercentage);
    image1Alpha = (255 * sliderPercentage) / 100;
            
    laString_Destroy(&sliderValueLabelString);
    sliderValueLabelString = laString_CreateFromCharBuffer(value, &ArialUnicodeMS);
    laLabelWidget_SetText(SliderValueLabelWidget, sliderValueLabelString);
        
    laLayer_SetAlphaAmount(context->activeScreen->layers[IMAGE1_LAYER_ID], image1Alpha);
    laLayer_SetAlphaAmount(context->activeScreen->layers[IMAGE2_LAYER_ID], 
    	(255 - image1Alpha));
}
...
{
    // Custom Action
    SYS_TMR_ObjectDelete(handleTimerPlay);
    SYS_TMR_ObjectDelete(handleTimerFF);
}
...
void ButtonWidget21_PressedEvent(laButtonWidget* btn)
{
    ...
    SYS_TMR_ObjectDelete(handleTimerFF);
    handleTimerFF = SYS_TMR_CallbackPeriodic(500, 1, Timer_Callback);
...
void ButtonWidget22_PressedEvent(laButtonWidget* btn)
{
    ...
    SYS_TMR_ObjectDelete(handleTimerPlay);
    handleTimerPlay = SYS_TMR_CallbackPeriodic(2000, 1, Timer_Callback);
...
// ButtonWidgetHelpTT - ReleasedEvent
...
// ButtonWidgetHomeTT - ReleasedEvent
...
// ButtonWidgetNextTT - ReleasedEvent
