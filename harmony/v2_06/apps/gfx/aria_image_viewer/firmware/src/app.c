...
#define IMAGE_WIDTH 2716
#define IMAGE_HEIGHT 1810
...
static n2d_buffer_t layer0, src0, src1, src2, src3, stage, alpha;
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        {                
            ...
            PLIB_GLCD_LayerGlobalAlphaSet( GLCD_ID_0, 0, 0xff );
            PLIB_GLCD_LayerDestBlendFuncSet(GLCD_ID_0, 0, GLCD_LAYER_DEST_BLEND_BLACK );
            PLIB_GLCD_LayerSrcBlendFuncSet(GLCD_ID_0, 0, GLCD_LAYER_SRC_BLEND_WHITE );
            PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 0, (uint32_t)GFX_GLCD_LAYER0_BASEADDR);

            /* initialize gui layer */
            PLIB_GLCD_LayerGlobalAlphaSet( GLCD_ID_0, 1, 0x00 );
            PLIB_GLCD_LayerDestBlendFuncSet(GLCD_ID_0, 1, GLCD_LAYER_DEST_BLEND_INV_SRC );
            PLIB_GLCD_LayerSrcBlendFuncSet(GLCD_ID_0, 1, GLCD_LAYER_SRC_BLEND_ALPHA_SRC );
            PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 1, (uint32_t)GFX_GLCD_LAYER1_BASEADDR);
                           
            /* initialize help layer */
            PLIB_GLCD_LayerGlobalAlphaSet( GLCD_ID_0, 2, 0xff );
            PLIB_GLCD_LayerDestBlendFuncSet(GLCD_ID_0, 2, GLCD_LAYER_DEST_BLEND_INV_GBL );
            PLIB_GLCD_LayerSrcBlendFuncSet(GLCD_ID_0, 2, GLCD_LAYER_SRC_BLEND_ALPHA_GBL );
            PLIB_GLCD_LayerBaseAddressSet(GLCD_ID_0, 2, (uint32_t)GFX_GLCD_LAYER2_BASEADDR);
            
            /* create GPU structures for GLCD layer 0 (dest) and scratch buffer (src) for image */
            layer0.format = N2D_RGBA8888;
            layer0.gpu = KVA_TO_PA(0xA8000000);
            ...
            src0.gpu = KVA_TO_PA(0xA97AFC00);
            src0.memory = (void*)0xA97AFC00;
            ...
            src1.gpu = KVA_TO_PA(0xAAA707E0);
            src1.memory = (void*)0xAAA707E0;
            ...
            src2.gpu = KVA_TO_PA(0xABD313C0);
            src2.memory = (void*)0xABD313C0;
            src2.width = IMAGE_WIDTH;
            src2.height = IMAGE_HEIGHT;
            src2.orientation = orientation;
            src2.stride = src2.width * 32 / 8;
      
            src3.format = N2D_RGBA8888;
            src3.gpu = KVA_TO_PA(0xACFF1FA0);
            src3.memory = (void*)0xACFF1FA0;
            src3.width = IMAGE_WIDTH;
            src3.height = IMAGE_HEIGHT;
            src3.orientation = orientation;
            src3.stride = src3.width * 32 / 8;
            
            /* scratch buffer for rotation */
            stage.format = N2D_RGBA8888;
            stage.gpu = KVA_TO_PA(0xA8BB8000);
            stage.memory = (void*)0xA8BB8000;
            ...
            for ( i=255; i>0; i--)
            {            
                PLIB_GLCD_LayerGlobalAlphaSet( GLCD_ID_0, 2, i );
                _DelayMS(10);
            }
...
//    float curYDist;//, speedY;
    ...
    switch(evt->touchID)
    {
        ...
        {            
            ...
            if ( ! touch1_down )
            {
                curTickCount = SYS_TMR_TickCountGet();
                ...
                if ( deltaTime != 0 )
                {
                    speed = (float)curDist / deltaTime;
                }               
               
                /* check for image swipe condition */
                if ( speed > 0.5 && deltaTime < 400 && curDist > 150  )
                {
                    n2d_uint32_t alphaValue=0x00;
                    int32_t direction = (evt->x > last0.x ? 0 : 1 );
                    while (alphaValue < 0x0f)
                    {
                        /* Init the alpha. */
                        memset(alpha.memory, alphaValue, alpha.stride * alpha.height);

                        /* Init the target buffer. */
            //            n2d_blit(&layer0, N2D_NULL, src, N2D_NULL, N2D_BLEND_NONE);
...
void APP_OrientButtonRelease( void )
{
    ...
    layer0.orientation = orientation;
    rect0.x = left; rect0.y = top; rect0.width = width; rect0.height = height;
    n2d_blit(&stage, N2D_NULL, src, &rect0, N2D_BLEND_NONE);
...
{    
    /* show help display */
    PLIB_GLCD_LayerGlobalAlphaSet( GLCD_ID_0, 2, 200 );
