...
uint8_t APP_MAKE_BUFFER_DMA_READY readBuffer[2][APP_READ_BUFFER_SIZE];
...
void APP_ProcessSwitchPress(APP_USB_DEVICE_OBJECT* deviceObject)
{
    ...
    {
        bspSwitch = APP_USB_SWITCH_1; 
    }
    else
    {
        bspSwitch = APP_USB_SWITCH_2;
    }
    if(BSP_SWITCH_STATE_PRESSED == (BSP_SwitchStateGet(bspSwitch)))
    {
