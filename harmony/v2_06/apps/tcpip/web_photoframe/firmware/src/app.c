...
void APP_SYSFS_EventHandler(SYS_FS_EVENT event, void * eventData, uintptr_t context)
{
    ...
    s_appData.totalFiles = 0;

    s_appData.fileHandle = SYS_FS_HANDLE_INVALID;
}

/*******************************************************************************
  Function:
    void APP_BuildImageString ( void )

  Remarks:
  Builds the string which has all the image names stored in the sdcard.
  * This string is placed in the index.htm file.
 */
void APP_BuildImageString(void)
{
    int i = 0;
    ...
    for (i = 0; i < s_appData.totalFiles; i++)
    {
        memcpy(otherString, &s_appData.fileNameTable[i].path, 40);

        if (i == 0)
            strcpy((char *)image_string, dot_start);
        else
            strcat((char *)image_string, dot_start);

        strcat((char *)image_string, otherString);

        if (i < s_appData.totalFiles-1)
            strcat((char *)image_string, dot_end);
        else
            strcat((char *)image_string, dot_end_last);
    }
}
...
{
    SYS_FS_RESULT ret;
    s_appData.dirHandle = SYS_FS_DirOpen(fname);
    if(s_appData.dirHandle == SYS_FS_HANDLE_INVALID)
    {
    ...
    *dir_count = 0;
    ...
    do
    {
        if(s_appData.totalFiles < APP_DISK_MAX_FILES)
        {
            ret = SYS_FS_DirRead(s_appData.dirHandle,&s_appData.dirstat);

            // End of this directory
            if(s_appData.dirstat.fname[0] == '\0')
            {
                break;
            }
            ...
            {
                if(s_appData.dirstat.fattrib != SYS_FS_ATTR_DIR)
                {
                    if(APP_IsSupportedFile(s_appData.dirstat.fname))
                    {
                        strcpy(s_appData.fileNameTable[s_appData.totalFiles].path, fname);

                        strcat(s_appData.fileNameTable[s_appData.totalFiles].path, "/");

                        strcat(s_appData.fileNameTable[s_appData.totalFiles].path, s_appData.dirstat.fname);

                        (s_appData.totalFiles)++;
                    }
                }
                else if(s_appData.dirstat.fattrib == SYS_FS_ATTR_DIR && s_appData.dirstat.fname[0] != '.') // Skip ".\" and "..\" directories
                {
                    if(*dir_count < APP_DISK_MAX_DIRS)
                    {
                        dir_table[*dir_count]=s_appData.dirstat;
                        (*dir_count)++;
                    }
                }
            }
    ...
    SYS_FS_DirClose(s_appData.dirHandle);
    return ret;
}
...
void APP_Tasks(void)
{
	...
	case APP_STATE_FS_MOUNT:
    {
        APP_TraverseAllFiles(s_appData.rootNode, true);

        index_file = SYS_FS_FileOpen("/mnt/myDrive1/index.htm", SYS_FS_FILE_OPEN_WRITE_PLUS);
        SYS_FS_FileSeek(index_file, 0, SYS_FS_SEEK_SET);
        while (strncmp(read_buffer, "pics", 4) != 0)
        {
            SYS_FS_FileStringGet(index_file,read_buffer,512);
            file_pos = SYS_FS_FileTell(index_file);
        }

        SYS_FS_FileSeek(index_file, file_pos, SYS_FS_SEEK_SET);
        /* Here is where we have to build the string of images. */
        for (i = 0; i < 999; i++)
        {
            image_string[i] = ' ';
        }
        ...
        image_string[999] = '\0';
        SYS_FS_FileStringPut(index_file, (const char *)image_string);
        SYS_FS_FileSeek(index_file, file_pos, SYS_FS_SEEK_SET);

        APP_BuildImageString();

        SYS_FS_FileStringPut(index_file, (const char*)image_string);

        SYS_FS_FileClose(index_file);
