...
HTTP_IO_RESULT TCPIP_HTTP_GetExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    // If it's the COLOR updater file.
    else if(!memcmp(filename, "color.cgi", 8))
    {
        // Determine which LED to toggle.
        colorStr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"color");
        sscanf((const char*)colorStr, "%02X%02X%02X", &red, &green, &blue);
...
void TCPIP_HTTP_Print_color(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    num = 0;
    // Print the output
    TCPIP_TCP_Put(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (num?'1':'0'));
