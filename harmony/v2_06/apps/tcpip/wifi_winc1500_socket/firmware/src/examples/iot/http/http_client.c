...
int http_client_init(struct http_client_module *const module, struct http_client_config *config, char *recv_buf)
{
	...
	/* recv buffer */
	module->config.recv_buffer = recv_buf;
...
void http_client_socket_resolve_handler(uint8_t *doamin_name, uint32_t server_ip)
{
	...
	SYS_CONSOLE_PRINT("resolve_cb: %s IP address is %d.%d.%d.%d\r\n\r\n", doamin_name,
			(int)IPV4_BYTE(server_ip, 0), (int)IPV4_BYTE(server_ip, 1),
			(int)IPV4_BYTE(server_ip, 2), (int)IPV4_BYTE(server_ip, 3));
