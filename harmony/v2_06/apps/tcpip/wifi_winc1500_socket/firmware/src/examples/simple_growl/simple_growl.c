/*******************************************************************************
  ...
  Description:
    This demo performs the following steps:
        ...
        5) WINC1500 will send a specific notification to a registered Android (NMA) or iOS (PROWL) device
    ...
    More information about this example:
        It supports sending Growl notifications to the following servers.
...
/*
 * Send a specific notification to a registered Android (NMA) or iOS (PROWL) device.
 */
static void growl_send_message_handler(void)
{
    ...
    //NMI_GrowlSendNotification(PROWL_CLIENT, (uint8_t *)"Growl_Sample", (uint8_t *)"Growl_Event", (uint8_t *)"growl_test", PROWL_CONNECTION_TYPE); // send by PROWL
    SYS_CONSOLE_PRINT("Growl message sent\r\n");
}

/*
 * Callback to get the Wi-Fi status update.
 *
 * msgType: type of Wi-Fi notification. Possible types are:
    ...
    case M2M_WIFI_REQ_DHCP_CONF:
	{
		...
		if (s_is_ap) {
			...
			SYS_CONSOLE_PRINT("wifi_cb: M2M_WIFI_REQ_DHCP_CONF: WINC1500's IP is %u.%u.%u.%u\r\n",
				pu8IPAddress[0], pu8IPAddress[1], pu8IPAddress[2], pu8IPAddress[3]);
			NMI_GrowlInit((uint8_t *)NMA_API_KEY, (uint8_t *)PROWL_API_KEY);
