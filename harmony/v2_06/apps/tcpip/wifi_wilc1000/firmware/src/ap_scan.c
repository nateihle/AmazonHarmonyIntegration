...
extern uint16_t WDRV_ScanStart(void);
...
static void APP_ScanResultDisplay(uint16_t index, WDRV_SCAN_RESULT *p_scanResult)
{
    uint8_t i;
...
void APP_WIFI_Tasks(void)
{
	...
	case APP_TCPIP_TRANSACT:
        ...
        switch(ScanState) {
            ...
            case 1:
                while(WDRV_EXT_IsScanComplete() != true) ;                
                ScanState = 2;
            break;
                
            case 2:    
                if (WDRV_EXT_ScanIsInProgress() == false)
                {
                    ...
                    for (cntr = 0; cntr < numberOfResults; cntr++)
                    {
                        WDRV_EXT_ScanResultGet(cntr, &scanResult);
                        APP_ScanResultDisplay(cntr, &scanResult);
                    }
                }
                break;
                }
...
{
    SYS_CONSOLE_MESSAGE("\r\n======================================");
    SYS_CONSOLE_MESSAGE("\r\n*** WILC1000 FreeRTOS AP Scan Demo ***");
    SYS_CONSOLE_MESSAGE("\r\n======================================\r\n");
}
