...
extern uint32 nmi_get_rfrevid(void);
extern sint8 m2m_wifi_get_mac_address(uint8 *pu8MacAddr0, uint8 *pu8MacAddr1);
extern sint8 m2m_wifi_get_otp_mac_address(uint8 *pu8MacAddr, uint8 * pu8IsValid);
...
static void APP_Get_MacAddress_Chip_Info(void);
...
static void APP_Get_MacAddress_Chip_Info(void)
{
    ...
    uint8_t mac_addr[6];
    uint8_t read_mac_addr[6];

	/* Get MAC Address from OTP. */
	m2m_wifi_get_otp_mac_address( (uint8_t *)mac_addr, &u8IsMacAddrValid);
	if (!u8IsMacAddrValid) {
		SYS_CONSOLE_PRINT("Cannot find MAC Address from OTP memory\r\n");
	} else {
		SYS_CONSOLE_PRINT("OTP MAC Address : \r\n");
	}

	/* Get MAC Address. */
	m2m_wifi_get_mac_address((uint8_t *)mac_addr, (uint8_t *)read_mac_addr );
...
{
    SYS_CONSOLE_MESSAGE("\r\n===========================================");
    SYS_CONSOLE_MESSAGE("\r\n*** WILC1000 MAC Address Chip Info Demo ***");
    SYS_CONSOLE_MESSAGE("\r\n===========================================\r\n");
}
