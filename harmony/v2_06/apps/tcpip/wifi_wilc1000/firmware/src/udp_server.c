...
void APP_WIFI_Tasks(void)
{
	...
	case APP_TCPIP_OPENING_SERVER:
		{
            ...
            s_appData.socket = TCPIP_UDP_ServerOpen(IP_ADDRESS_TYPE_IPV4, SERVER_PORT, 0);
    ...
    case APP_TCPIP_SERVING_CONNECTION:
    {
        ...
        wMaxGet = TCPIP_UDP_GetIsReady(s_appData.socket);	// Get TCP RX FIFO byte count
        wMaxPut = TCPIP_UDP_PutIsReady(s_appData.socket);	// Get TCP TX FIFO free space
        ...
        for(w = 0; w < wMaxGet; w += sizeof(AppBuffer))
        {
            ...
            int rxed = TCPIP_UDP_ArrayGet(s_appData.socket, AppBuffer, wCurrentChunk);
            ...
            TCPIP_UDP_ArrayPut(s_appData.socket, AppBuffer, wCurrentChunk);
