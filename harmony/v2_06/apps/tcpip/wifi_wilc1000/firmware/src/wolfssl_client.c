		...
		case TCPIP_DNS_RES_OK:
		{
			...
			else
			{
				SYS_CONSOLE_PRINT("DNS Resolved IPv6 Address: %x:%x:%x:%x:%x:%x:%x:%x for host '%s'\r\n",
					htons(s_appData.address.v6Add.w[0]),
					htons(s_appData.address.v6Add.w[1]),
					htons(s_appData.address.v6Add.w[2]),
					htons(s_appData.address.v6Add.w[3]),
					htons(s_appData.address.v6Add.w[4]),
					htons(s_appData.address.v6Add.w[5]),
					htons(s_appData.address.v6Add.w[6]),
					htons(s_appData.address.v6Add.w[7]),
					s_appData.host);
			}
...
static void start_socket(void)
{
	...
	if (s_appData.queryState == 4)
	{
		SYS_CONSOLE_PRINT("Starting TCP/IPv4 Connection to : %d.%d.%d.%d port  '%d'\r\n",
					...
					s_appData.address.v4Add.v[3],
					s_appData.port);
		s_appData.socket = NET_PRES_SocketOpen(0, 
					NET_PRES_SKT_UNENCRYPTED_STREAM_CLIENT, 
					IP_ADDRESS_TYPE_IPV4, 
					s_appData.port, 
					(NET_PRES_ADDRESS *)&s_appData.address,
					NULL);
	}
	else
	{
	...
	NET_PRES_SocketWasReset(s_appData.socket);
