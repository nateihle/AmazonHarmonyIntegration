...
void APP_WIFI_Tasks(void)
{
		...
		if (g_wifi_link_up)
		{
            if (APP_Send_Packet)
            {
                ...
                if (result == TCPIP_DNS_RES_NAME_IS_IPADDRESS)
                {
                    ...
                    s_appData.socket = TCPIP_UDP_ClientOpen(IP_ADDRESS_TYPE_IPV4,
                                                          port,
    ...
    case APP_TCPIP_WAIT_FOR_CONNECTION:
        {
            if (!TCPIP_UDP_IsConnected(s_appData.socket))
            {
                s_appData.state = APP_TCPIP_TRANSACT;
                break;
            }
            if(TCPIP_UDP_PutIsReady(s_appData.socket) == 0)
            {
                s_appData.state = APP_TCPIP_TRANSACT;
                break;
            }
            TCPIP_UDP_ArrayPut(s_appData.socket, (uint8_t*)APP_Message_Buffer, strlen(APP_Message_Buffer));
            TCPIP_UDP_Flush(s_appData.socket);
            s_appData.mTimeOut = SYS_TMR_SystemCountGet() + (SYS_TMR_TickCounterFrequencyGet() * 20);
        ...
        case APP_TCPIP_WAIT_FOR_RESPONSE:
        {
            ...
            if (SYS_TMR_SystemCountGet() > s_appData.mTimeOut)
            {
                SYS_CONSOLE_MESSAGE("\r\nTimout waiting for response\r\n");
                TCPIP_UDP_Close(s_appData.socket);
                s_appData.state = APP_TCPIP_TRANSACT;
                break;
            }
            ...
            if (TCPIP_UDP_GetIsReady(s_appData.socket))
            {
                TCPIP_UDP_ArrayGet(s_appData.socket, (uint8_t*)buffer, sizeof(buffer) - 1);
                TCPIP_UDP_Discard(s_appData.socket);
