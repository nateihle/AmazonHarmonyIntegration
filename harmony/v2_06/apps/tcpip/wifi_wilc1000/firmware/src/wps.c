...
void APP_WIFI_Start_WPS(void)
{
	int8_t ret;

#define WLAN_DEVICE_NAME    			"WILC1000_WPS" /* WILC1000's Device Name in WPS */
/** WPS PIN number */
...
#define MAIN_WPS_PUSH_BUTTON_FEATURE    true
	...
	if (!MAIN_WPS_PUSH_BUTTON_FEATURE) {
		SYS_CONSOLE_PRINT("WPS PIN (%s) Method In Progress\r\n", MAIN_WPS_PIN_NUMBER);
		m2m_wifi_wps(WPS_PIN_TRIGGER, (const char *)MAIN_WPS_PIN_NUMBER);
	}

	/* WPS Push Button Method Enabled. */
	if (MAIN_WPS_PUSH_BUTTON_FEATURE) {
		SYS_CONSOLE_PRINT("WPS PUSH Button Method In Progress\r\n");
