...
static int percentVolume, lastPercentVolume = -1;
...
//                 return;           
            percentVolume = (100*volData.potValue)/1023;
            ...
            if (percentVolume != lastPercentVolume)
            {
                int diff = abs(percentVolume-lastPercentVolume);
                if (diff > 1)
                {
                    DigitalMeterValueSet32(GFX_VOLUME_VALUE, percentVolume, true);
                    lastPercentVolume = percentVolume;

                    if (appData.codecData.codecClient.handle != DRV_HANDLE_INVALID)
                    {
                        mVolume = 85*mNewVolume/100;  // scale for AK4954, to avoid overload
