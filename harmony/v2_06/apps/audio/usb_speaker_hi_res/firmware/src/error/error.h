...
//Alternative implemenation of assert())
#include "error/assert_to_display.h"
...
#undef assert
  ...
  #define assert(expression)   \
      ...
      //if ((!expression))  SYS_DEBUG_BreakPoint();
#else
    #define assert(expression) \
      ((void)( (expression) ? 0 : assert_to_display(#expression,__LINE__,__FILE__)))
#endif //__DEBUG
...
//void bt_oem_assert(const char* file, int line); //cdbt Bluetooth Statck
