...
typedef enum
{
    ...
    APP_USB_SPEAKER_PLAYBACK_STEREO_96KHZ = 1
...
typedef struct
{
    ...
    char padding[5];      //1355+5 = 1360 = 16*85 bytes  (align 16 bytes))
}APP_PLAYBACK_BUFFER;
...
//        .queueSizeWrite =16,
//    };
//system_init.c: set from the value in system_init
#define APP_PLAYBACK_QUEUE_BUFFER_SIZE 16 
...
typedef struct
{
    APP_PLAYBACK_BUFFER __attribute__((coherent)) __attribute__((aligned(16)))  
            playbackBuffer[APP_PLAYBACK_QUEUE_BUFFER_SIZE];
    ...
    uint32_t usbReadCompleteCnt;       //Total number of reads completed
    uint32_t usbReadQueueCnt;          //#of usb Reads queued
    uint32_t codecWriteQueueCnt;       //#of codec writes queued
    uint32_t codecWriteCompleteCnt;    //Total number of codec writes completed
