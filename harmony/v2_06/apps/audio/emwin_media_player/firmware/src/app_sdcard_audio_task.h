...
typedef bool (*APP_SDCARD_AUDIO_EVENT_NOTIFY)(DISP_CMD);
...
{            
    ...
    APP_SDCARD_AUDIO_THUMBDRIVE_FS_UNMOUNT,
    /* Set the current drive */
    APP_SDCARD_AUDIO_CARD_CURRENT_DRIVE_SET,
            
    APP_SDCARD_AUDIO_STATE_RUNNING, 
    APP_SDCARD_AUDIO_USB_BUS_ENABLE,
    APP_SDCARD_AUDIO_WAIT_FOR_USB_BUS_ENABLE_COMPLETE,    
...
typedef enum
{
    ...
    APP_SDCARD_AUDIO_PLAYER_STATE_STOP,
...
typedef struct
{
    ...
    bool isLoopSingleTrack;
    bool isLoopTrackList;
    bool isShuffleOn;
    bool isPlayerRunning;
    uint32_t nTotalBytesPlayed;
    uint16_t playerTrackTotalTime;
    uint16_t playerBitrate;
        
    APP_SDCARD_AUDIO_PLAYER_COMMAND pressCMD;
    APP_SDCARD_AUDIO_STREAM currentStreamType;
    APP_SDCARD_AUDIO_EVENT_NOTIFY eventNotify;
...
typedef struct
{
    ...
    bool mountedDeviceType;
    bool isThumbDriveConnected;    
    bool isFSMountedOnThumbDrive;    
...
void APP_SDCARD_AUDIO_UpdatePlayerRunningStatus(bool);
void APP_SDCARD_AUDIO_SeekTrackPosition(int32_t position);
