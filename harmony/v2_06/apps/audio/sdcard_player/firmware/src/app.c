...
static void _APP_BUTTON_PRESS_DebounceTimerHandler ( uintptr_t context, uint32_t currTick );
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_INIT:
        {
            ...
            if (appInitialized)
            {
                if(SYS_TMR_CallbackPeriodic ( 250, 1, _APP_BUTTON_PRESS_DebounceTimerHandler )
...
static void _APP_BUTTON_PRESS_Tasks(void)
{
    if(BSP_SWITCH_STATE_PRESSED == BSP_SwitchStateGet(APP_AUDIO_SRC_SWITCH))
    {
