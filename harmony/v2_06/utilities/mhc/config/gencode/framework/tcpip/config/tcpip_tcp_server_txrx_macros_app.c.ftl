...
static char ${APP_NAME?lower_case}MsgToClient[] = "${("CONFIG_TCPIP_TCP_SERVER_TXRX_TRANSMIT_STRING" + "${HCONFIG_APP_INSTANCE}")?eval}\n\r";
static uint8_t ${APP_NAME?lower_case}MsgFromClient[${("CONFIG_TCPIP_TCP_SERVER_TXRX_RCV_BUFFER_SIZE" + "${HCONFIG_APP_INSTANCE}")?eval}];
	...
	switch (${APP_NAME?lower_case}Data.txrxTaskState)
	{
        case ${APP_NAME?upper_case}_TCPIP_WAIT_FOR_IP:
        {
            ...
            for (i = 0; i < ${APP_NAME?lower_case}_nNets; i++)
            {
                ...
                {
                    ${APP_NAME?lower_case}Data.txrxTaskState = ${APP_NAME?upper_case}_TCPIP_OPENING_SERVER;
                }
            }
            break;
        }
        case ${APP_NAME?upper_case}_TCPIP_OPENING_SERVER:
        {
            ${APP_NAME?lower_case}Data.socket = TCPIP_TCP_ServerOpen(IP_ADDRESS_TYPE_IPV4, ${APP_NAME?lower_case}Data.port, 0);
        ...
        case ${APP_NAME?upper_case}_TCPIP_WAIT_FOR_CONNECTION:
        {
            ...
            {
                // We got a connection
				TCPIP_TCP_ArrayPut(${APP_NAME?lower_case}Data.socket, ${APP_NAME?lower_case}MsgToClient, sizeof(${APP_NAME?lower_case}MsgToClient));
        ...
        case ${APP_NAME?upper_case}_TCPIP_WAIT_FOR_RESPONSE:
        {
            ...
            {
                TCPIP_TCP_ArrayGet(${APP_NAME?lower_case}Data.socket, ${APP_NAME?lower_case}MsgFromClient, sizeof(${APP_NAME?lower_case}MsgFromClient) - 1);
            }
        }
        break;
...
<#if ("CONFIG_TCPIP_TCP_SERVER_TXRX" + "${HCONFIG_APP_INSTANCE}")?eval>
	${APP_NAME?lower_case}Data.port = ${("CONFIG_TCPIP_TCP_SERVER_TXRX_PORT" + "${HCONFIG_APP_INSTANCE}")?eval};
</#if>
</#macro>
