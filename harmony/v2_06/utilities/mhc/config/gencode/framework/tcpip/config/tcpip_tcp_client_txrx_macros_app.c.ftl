...
<#if ("CONFIG_TCPIP_TCP_CLIENT_TXRX" + "${HCONFIG_APP_INSTANCE}")?eval>
static char ${APP_NAME?lower_case}ServerIP[] = "${("CONFIG_TCPIP_TCP_CLIENT_TXRX_SERVER_IP" + "${HCONFIG_APP_INSTANCE}")?eval}";
static char ${APP_NAME?lower_case}Msg[] = "${("CONFIG_TCPIP_TCP_CLIENT_TXRX_TRANSMIT_STRING" + "${HCONFIG_APP_INSTANCE}")?eval}";
static uint8_t ${APP_NAME?lower_case}ResponseBuffer[${("CONFIG_TCPIP_TCP_CLIENT_TXRX_RCV_BUFFER_SIZE" + "${HCONFIG_APP_INSTANCE}")?eval}];
	...
	switch (${APP_NAME?lower_case}Data.txrxTaskState)
	{
        case ${APP_NAME?upper_case}_TCPIP_WAIT_FOR_IP:
        {
            ...
            for (i = 0; i < ${APP_NAME?lower_case}_nNets; i++)
            {
                ...
                {
                    ${APP_NAME?lower_case}Data.txrxTaskState = ${APP_NAME?upper_case}_TCPIP_WAITING_FOR_COMMAND;
                }
            }
            break;
        }
...
<#if ("CONFIG_TCPIP_TCP_CLIENT_TXRX" + "${HCONFIG_APP_INSTANCE}")?eval>
	${APP_NAME?lower_case}Data.port = ${("CONFIG_TCPIP_TCP_CLIENT_TXRX_PORT" + "${HCONFIG_APP_INSTANCE}")?eval};
</#if>
</#macro>
