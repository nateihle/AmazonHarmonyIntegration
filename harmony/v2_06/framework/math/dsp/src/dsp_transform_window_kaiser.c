...
Microchip DSP Library
    Kaiser Windows on a Vector

  Company:
    Microchip Technology Inc.

  File Name:
    dsp_transform_window_kaiser.c
  ...
  Description:
    ...
    fractional format respectively. The Hanning Window follows the equation:
...
void DSP_TransformWindow_Kaiser16(int16_t *OutVector, int16_t *InVector, int N)
{
	...
	for (i = 0; i < N; i++)
	{
            Window = 0.402 - 0.498 * cos((2*M_PI * i) / N)+ 0.098 * cos(4*M_PI*i/N) + 0.001 * cos(6*M_PI*i/N); // window function
