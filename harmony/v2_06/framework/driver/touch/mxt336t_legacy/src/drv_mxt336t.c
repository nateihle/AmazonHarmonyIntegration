...
#include "driver/touch/mxt336t_legacy/src/drv_mxt336t_local.h"
                                           ...
                                           const SYS_MODULE_INIT * const init )
{
    ...
    pDrvInstance->interruptSource       = pInit->interruptSource;
    pDrvInstance->interruptChannel      = pInit->interruptChannel;
    pDrvInstance->interruptPin          = pInit->interruptPin;
    /* */
...
void DRV_MXT336T_Tasks ( SYS_MODULE_OBJ object )
{
    ...
    switch (pDrvObject->deviceState)
    {
        ...
        case DRV_MXT336T_DEVICE_STATE_READY: /* Driver ready state */
            ...
            /* Check for ~CHG line asserted in case interrupt was missed */
            if (PLIB_PORTS_PinGet(PORTS_ID_0, pDrvObject->interruptChannel, pDrvObject->interruptPin) == false)
            {
