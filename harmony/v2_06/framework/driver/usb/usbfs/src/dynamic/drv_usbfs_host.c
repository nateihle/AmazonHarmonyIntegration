...
)
{
    ...
    {
        /* A full duplex endpoint has 4 entries, 2 per EP direction */
...
)
{
    ...
    if((pPipe != NULL) && (pPipe != (DRV_USBFS_HOST_PIPE_OBJ *)DRV_USBFS_HOST_PIPE_HANDLE_INVALID))
    {
...
void DRV_USBFS_HOST_IRPCancel(USB_HOST_IRP * pInputIRP)
{
    ...
    if((pIRP != NULL) && (pIRP->pipe != DRV_USBFS_HOST_PIPE_HANDLE_INVALID))
    {
        ...
        if(!(pUSBDrvObj->inInterruptContext))
        {
             ...
             * protected by a mutex. We do not want the status of the IRP to
            ...
            {
                /* We cannot proceed. Set a flag saying an error has occurred.
                 * */
                SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "\r\nUSBFS Driver: Mutex lock failed");
        ...
        {
            /* There was no error */
            if(pIRP->status > USB_HOST_IRP_STATUS_COMPLETED_SHORT)
            {
                /* This means the IRP is pending or in progress. Search for the
                 * IRP in the queue. irpCancel will be true if the IRP to be
                 * cancelled was found in the queue. */
                ...
                else
                {
                    ...
                    while(pIteratorIRP != NULL)
                    {
                        ...
                        {
                            /* IRP to be cancelled has been found. The next and
                             * previous pointers in the doubly linked list need
                             * to be manipulated to removed this IRP from the
                             * queue. */
                ...
                if(irpCancel == true)
                {
                    ...
                    {
                        /* If the irp is already in progress then we set the
                         * temporary state. This will get caught in
                    ...
                    {
                        /* If the IRP is not in progress, then we can terminate
                         * it immediately. Invoke the callback. */
                ...
                {
                    /* Either no IRP at all in the Pipe or particular IRP not
                     * found */
...
/* Function:
  ...
  Summary:
    This function advances the control transfer.
...
)
{
    ...
    bool endIRP = false;
    DRV_USBFS_HOST_SW_EP * softwareEP = NULL;
    DRV_USBFS_HOST_TRANSFER_GROUP * transferGroup = NULL;
    ...
    if(pIRP->tempState == DRV_USBFS_HOST_IRP_STATE_ABORTED)
    {
        ...
        ((pUSBDrvObj->transferGroup[softwareEP->transferType]).currentPipe)->irpQueueHead = (softwareEP->pIRP)->next;
        ...
        if((transferGroup->currentPipe) ->next != NULL)
        {
            /* This is not the last PIPE */
            transferGroup->currentPipe = transferGroup->currentPipe->next;
        }
        else
        {
    ...
    {
        /* IRP is not aborted. We should continue to process this transfer */
        ...
        switch(pIRP->tempState)
        {
            case DRV_USBFS_HOST_IRP_STATE_SETUP_STAGE:
                ...
                pBDT->byte[0] = (USBFS_UOWN|USBFS_DTS_ENABLE);
            ...
            case DRV_USBFS_HOST_IRP_STATE_HANDSHAKE:
                /* Send the Handshake Stage */
                {
                    ...
                    pBDT->shortWord[1]  = 0;
                    pBDT->word[1] = 0;
                    pBDT->byte[0] = (USBFS_UOWN|USBFS_DATA1_PID|USBFS_DTS_ENABLE);
        ...
        if(endIRP == true)
        {
            ...
            (transferGroup->currentPipe)->irpQueueHead = (softwareEP->pIRP)->next;
    ...
    if((pipeHandle != DRV_USBFS_HOST_PIPE_HANDLE_INVALID) && (pipeHandle != (DRV_USBFS_HOST_PIPE_HANDLE)(NULL)))
    {
        ...
        if(pPipe->inUse == true)
        {
            ...
            if(!thereWasAnError)
            {
                ...
                if(!thereWasAnError)
                {
                    /* Pipe is not orphan and is part of the transfer group.
                     ...
                     * detached, then we could potentially land in a situatino
                     * where the IRP temp state says aborted, but the TRNIF will
                     * never occur and then IRP will never be aborted. Deferring
                     * the IRP abort allows the USB module to complete any
                    ...
                    while(pIRP != NULL)
                    {
                        ...
                        if((pIRP->status == USB_HOST_IRP_STATUS_IN_PROGRESS) && (pUSBDrvObj->isDeviceDeenumerating == false))
                        {
                        ...
                        else
                        {
                             ...
                             * needs to be aborted. We cannot wait for TRNIF as
                             * there will be no TRNIF post device detach */
...
)
{
    ...
    if((handle != DRV_HANDLE_INVALID) && (pUSBDrvObj != NULL) && (pUSBDrvObj->isOpened))
    {
        ...
        if(OSAL_MUTEX_Lock(&pUSBDrvObj->mutexID, OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
        {
            ...
            for(pipeCount = 0; pipeCount < DRV_USBFS_HOST_PIPES_NUMBER; pipeCount++)
            {
                ...
                if(gDrvUSBHostPipeObj[pipeCount].inUse == false)
                {
                     ...
                     * mapping from the transfer type to the way the pipe
                     * bundles are organized is not a one to one mapping. We
                     * translate the pipe type to a local type. */
                    ...
                    /* The bandwidth required per transaction is calculated
                     * up front so that we dont have to calculate this in
                     * the interrupt context when the transaction is in
                     * progress */
            ...
            if(returnValue == DRV_USBFS_HOST_PIPE_HANDLE_INVALID)
            {
...
)
{
    ...
    pBDT->byte[0] = (USBFS_UOWN|USBFS_DTS_ENABLE) | (pipe->dataToggle << 6);
...
/* Function:
  ...
  Summary:
    This function sends a non control transfer token.
...
/* Function:
  ...
  Summary:
    This function process a non control transfer IRP.
...
)
{
    ...
    /* Get a pointer to the software endpoint */
    softwareEP = &pUSBDrvObj->drvUSBHostSWEp[pUSBDrvObj->numSWEpEntry];

    if(pIRP->tempState == DRV_USBFS_HOST_IRP_STATE_ABORTED)
    {
        /* This means that this IRP was aborted by the application while it was
         * in progress. Terminate it now. */

        softwareEP->tobeDone = false;

        /* Remove the IRP from pipe. */ 
        ((DRV_USBFS_HOST_PIPE_OBJ *)(softwareEP->pIRP->pipe))->irpQueueHead = (softwareEP->pIRP)->next;
    ...
    else
    {
        ...
        {
            /* This means this function was called from the TRNIF interrupt
             * after the token was sent. Check the transaction result */
            ...
            else
            {
                ...
                switch(lastTransactionResult)
                {
                    ...
                    case USB_TRANSACTION_DATA1:    
                        ...
                        else if((lastTransactionsize == pIRP->tempSize) || (lastTransactionsize == pipe->endpointSize))
                        {
                            if(pIRP->tempSize > pIRP->completedBytesInThisFrame)
                            {
...
/* Function:
  ...
  Summary:
    This function checks how many transactions of a control transfer can
...
)
{
     ...
     * The pipe data structure was updated with this information as part of pipe
     * creation */
...
/* Function:
  ...
  Summary:
    This function check if we have enough bandwidth to complete non control
    transaction in the frame.
...
void _DRV_USBFS_HOST_TransferPack (DRV_USBFS_OBJ * pUSBDrvObj)
{
    ...
    /* 1st time Current pipe (processing will start from this pipe) will be head
     * pipe. After that it will rotate through the Control pipe linked list */

    if(pTransferGroup->pipe != NULL)
    {
        if(pUSBDrvObj->drvUSBHostSWEp[0].tobeDone == true)
        {
            /* Transfer continuation from last frame. We only need to update the
             * bandwidth based on the control transfer packet size. */

            pControlIRP = (pTransferGroup->currentPipe)->irpQueueHead;

            /* Calling this function will update the globalBWConsumed field and
             * the tobeDone field. Because this is being called at the start of
             * the frame, we know that bandwidth will always get allocated. */
        ...
        else
        {
             ...
             * called,currentPipe will be equal to transfer group pipe.  From
             * then onwards,we will rotate through CONTROL pipes linked list for
             * the existing number of pipes.
             *
             * If IRP has been packed for a particular pipe, the current pipe
             * will be moved in ISR context after the IRP processing has been
             * done completely. This event of complete CONTROL IRP processing
             * may require multiple transactions spread over multiple USB frames
             * as well.
             * 
             * On next call to transfer scheduler, the pipe processing will
             * start from the updated current pipe. */

            /* Search for a CONTROL transfer to schedule till we have an IRP to
             * process or till we have run out of all open CONTROL transfer
             * pipes */
            ...
            for(loop = 0; loop < pTransferGroup->nPipes; loop++)
            {
                ...
                if(pControlIRP != NULL)
                {
                    /* So the PIPE has valid CONTROL IRP.  Analyze the bandwidth
                     * requirements for this IRP and calculate the amount of
                     * data to be processed in this USB frame.  The
                     * _DRV_USBFS_HOST_CalculateControlBW function will set the
                     * tobeDone flag true if the transaction could be scheduled
                     * in this frame.  */

                    _DRV_USBFS_HOST_CalculateControlBW(pUSBDrvObj, pTransferGroup, pControlIRP);
                    if(pUSBDrvObj->drvUSBHostSWEp[0].tobeDone == true)
                    {
                        /* We have successfully packed a CONTROL IRP.  The
                         * transfer scheduler processes 1 CONTROL IRP per frame.
                         * So we increment the IRP counter to 1 and break from
                         * the for() loop */
                        numIRPProcess = 1;
                        break;
                    }
                    ...
                    {
                        /* Not even 1 transaction was possible.  The code
    ...
    if((pTransferGroup->pipe != NULL) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER))
    {
        ...
        do
        {
            ...
            {
                /* ISOCHRONOUS IRP, if present, will be scheduled from this pipe */
            }
            else
            {
        ...
        for(loop = 0; loop < (pTransferGroup->nPipes) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER); loop++)
        {
            ...
            if((pisochronousIRP != NULL) && (piteratorPipe->intervalCounter == 0))
            {
                ...
                irpPacked = _DRV_USBFS_HOST_CalculateNonControlBW ( pUSBDrvObj, pTransferGroup, pisochronousIRP, USB_TRANSFER_TYPE_ISOCHRONOUS, numSWEpEntry);
                ...
                else
                {
                     ...
                     * because of bandwidth limitation. We will try the next
                     * ISOCHRONOUS pipe in the next iteration.  */
                }
            }
    ...
    if((pTransferGroup->pipe != NULL) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER))
    {
        ...
        for(loop = 0; loop < (pTransferGroup->nPipes) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER); loop++)
        {
            ...
            if((pinterruptIRP != NULL) && (piteratorPipe->intervalCounter == 0))
            {
                ...
                piteratorPipe->intervalCounter = piteratorPipe->bInterval;
                /* Check BW and pack IRP */
                irpPacked = _DRV_USBFS_HOST_CalculateNonControlBW ( pUSBDrvObj, pTransferGroup, pinterruptIRP, USB_TRANSFER_TYPE_INTERRUPT, numSWEpEntry);
    ...
    if((pTransferGroup->pipe != NULL) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER))
    {
         ...
         * 1st time current pipe will be same as pipe. After that current pipe
         * will move within the BULK pipe list as per IRP processed */
        for(loop = 0; loop < (pTransferGroup->nPipes) && (numIRPProcess < _DRV_USBFS_HOST_IRP_PER_FRAME_NUMBER) && (numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER); loop++)
        {
            ...
            if(pbulkIRP != NULL)
            {
                ...
                irpPacked = _DRV_USBFS_HOST_CalculateNonControlBW ( pUSBDrvObj, pTransferGroup, pbulkIRP, USB_TRANSFER_TYPE_BULK, numSWEpEntry);
                ...
                {
                    /* The IRP from this BULK pipe cannot be processed because
                     * of bandwidth limitation. We will try the next BULK pipe
...
)
{
    ...
    if(lastResult == 0)
    {
        ...
        {
            /* The code flow comes here when IRP processing is over and we need
             * to move to the next IRP that has been scheduled in this frame.
             * Increment the numSWEpEntry tracker to point to the next software
             * endpoint.*/

            numSWEpEntry = pUSBDrvObj->numSWEpEntry + 1;
        }

        if(numSWEpEntry < _DRV_USBFS_SW_EP_NUMBER)
        {

            /* Call appropriate transfer function to process.  Please note how
             * numSWEpEntry is not initialized to 0 here here in the for() loop. For
             * Frame expiry case numSWEpEntry will be initialized to 0 before only.
            ...
            {
                /* Obtain a pointer to the software endpoint being analyzed. */

                softwareEP = &pUSBDrvObj->drvUSBHostSWEp[numSWEpEntry];
                if((softwareEP->tobeDone) == true)
                {
                    ...
                    if(softwareEP->transferType == USB_TRANSFER_TYPE_CONTROL) 
                    {
                        /* Control transfer */
                        tokenSent = _DRV_USBFS_HOST_ControlXferProcess ( pUSBDrvObj, softwareEP->pIRP, (DRV_USBFS_TRANSACTION_RESULT) 0, 0);
                    }
                    else if((softwareEP->transferType == USB_TRANSFER_TYPE_ISOCHRONOUS) ||
                            (softwareEP->transferType == USB_TRANSFER_TYPE_INTERRUPT) ||
                            (softwareEP->transferType == USB_TRANSFER_TYPE_BULK))
                    {
                        /* Non Control transfers */
                        tokenSent = _DRV_USBFS_HOST_NonControlIRPProcess ( pUSBDrvObj, softwareEP->pIRP, (DRV_USBFS_TRANSACTION_RESULT) 0, 0);
                    }
                    else
                    {
                    ...
                    /* Note that we are not checking the tokenSent value here.
                     * If there was a IRP to be processed in the software EP,
                     * this would have been checked and sent. Noting that the
                     * token has been sent, we stop and wait for the token to
                     * complete */
        ...
        {
            /* If here means that we have processed all the software endpoint
             * and dont need to send any thing more in this frame. */
            tokenSent = false;
        }
    }
    else
    {
        /* The code flow comes here for every TRNIF generated for an IRP.  The
         * below code helps in processing for multiple transaction transfers as
         * per the USB transfer type.  */
        ...
        if(pUSBDrvObj->numSWEpEntry == 0)
        {
            ...
            tokenSent = _DRV_USBFS_HOST_ControlXferProcess ( pUSBDrvObj, softwareEP->pIRP, (DRV_USBFS_TRANSACTION_RESULT) lastResult, transactionSize);
        }
        else
        {
            if((pUSBDrvObj->drvUSBHostSWEp[pUSBDrvObj->numSWEpEntry].tobeDone == true))
            {
                ...
                tokenSent = _DRV_USBFS_HOST_NonControlIRPProcess ( pUSBDrvObj, softwareEP->pIRP, (DRV_USBFS_TRANSACTION_RESULT) lastResult, transactionSize);
            }
        }
    }
...
void _DRV_USBFS_HOST_Tasks_ISR(DRV_USBFS_OBJ * pUSBDrvObj)
{
    ...
    if((PLIB_USB_OTG_InterruptIsEnabled(usbID, USB_OTG_INT_ONE_MS_TIMEOUT)) && (PLIB_USB_OTG_InterruptFlagGet(usbID, USB_OTG_INT_ONE_MS_TIMEOUT)))
    {
...
void DRV_USBFS_HOST_ROOT_HUB_OperationEnable(DRV_HANDLE handle, bool enable)
{
    ...
    if((handle != DRV_HANDLE_INVALID) && (handle != (DRV_HANDLE)(NULL)) && (pUSBDrvObj->isOpened))
    {
        if(true == enable)
        {
...
USB_ERROR DRV_USBFS_HOST_ROOT_HUB_PortReset(DRV_HANDLE handle, uint8_t port)
{
    ...
    if((DRV_HANDLE_INVALID != handle) && (handle != (DRV_HANDLE)(NULL)))
    {
        ...
        {
            if(!pUSBDrvObj->isResetting)
            {
                /* Start the reset signal. Set the driver flag to indicate the
                 * reset signal is in progress. Enable the 1 millisecond timer
                 * interrupt to count the reset duration. Clear the reset
                 * duration counter. This counter will be updated in the
                 * interrupt. Start generating the reset signal. */
