...
static void DRV_SDCARD_TimerCallback
(
    uintptr_t context,
    uint32_t currTick
)
        ...
        case DRV_SDCARD_CMD_CHECK_RESP_TYPE:
            {
                ...
                if (spiRetVal == DRV_SPI_BUFFER_EVENT_COMPLETE)
                {
                    if (dObj->cmdResponse.response1.byte == DRV_SDCARD_MMC_FLOATING_BUS)
                    {
                        ...
                        {
                            dObj->cmdRespTmrState = false;
                            dObj->cmdRespTmrHandle = SYS_TMR_CallbackSingle(_DRV_SDCARD_FLOATING_BUS_TIMEOUT,
                                        (uintptr_t)&dObj->cmdRespTmrState,
                                        DRV_SDCARD_TimerCallback);
                            dObj->cmdRespTmrFlag = true;
                        }

                        if (dObj->cmdRespTmrState == true)
                        {
        ...
        case DRV_SDCARD_CMD_R1B_READ_BACK:
            {
                ...
                if (spiRetVal == DRV_SPI_BUFFER_EVENT_COMPLETE)
                {
                    ...
                    else
                    {
                        ...
                        {
                            dObj->cmdRespTmrState = false;
                            dObj->cmdRespTmrHandle = SYS_TMR_CallbackSingle(_DRV_SDCARD_R1B_RESP_TIMEOUT,
                                        (uintptr_t)&dObj->cmdRespTmrState,
                                        DRV_SDCARD_TimerCallback);
                            dObj->cmdRespTmrFlag = true;
                        }
        ...
        case DRV_SDCARD_INIT_SEND_ACMD41:
            {
                ...
                if (dObj->cmdState == DRV_SDCARD_CMD_EXEC_IS_COMPLETE)
                {
                    ...
                    else
                    {
                        ...
                        {
                            dObj->timerState = false;
                            dObj->timerHandle = SYS_TMR_CallbackSingle(_DRV_SDCARD_APP_CMD_RESP_TIMEOUT_IN_MS,
                                        (uintptr_t)&dObj->timerState,
                                        DRV_SDCARD_TimerCallback);
                            dObj->timerFlag = true;
                        }

                        if (dObj->timerState == true)
                        {
                            dObj->mediaInitState = DRV_SDCARD_INIT_ERROR;
                            dObj->timerFlag = false;
                        }
...
)
{
    ...
    switch (dObj->taskBufferIOState)
    {
        ...
        case DRV_SDCARD_TASK_PROCESS_READ:
            {
                ...
                if (dObj->cmdState == DRV_SDCARD_CMD_EXEC_IS_COMPLETE)
                {
                    ...
                    {
                        dObj->timerFlag = false;
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_READ_START_TOKEN;
                    }
                    else
                    {
                        /* Perhaps the card isn't initialized or present */
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_READ_WRITE_ABORT;
                    }
                }
                else if (dObj->cmdState == DRV_SDCARD_CMD_EXEC_ERROR)
                {
        ...
        case DRV_SDCARD_TASK_SPI_STATUS:
            {
                spiRetVal = DRV_SPI_BufferStatus (dObj->spiBufferHandle);
                if (spiRetVal == DRV_SPI_BUFFER_EVENT_COMPLETE)
                {
        ...
        case DRV_SDCARD_TASK_READ_START_TOKEN:
            {
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, dObj->pCmdResp, 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_READ_START_TOKEN_STATUS;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
                    dObj->timerFlag = false;
        ...
        case DRV_SDCARD_TASK_READ_START_TOKEN_STATUS:
            {
                ...
                {
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_READ_DATA;
                ...
                else
                {
                    if (dObj->timerFlag == false)
                    {
                        ...
                        dObj->timerState = false;
                        dObj->timerHandle = SYS_TMR_CallbackSingle(_DRV_SDCARD_READ_TIMEOUT_IN_MS,
                                (uintptr_t)&dObj->timerState,
                                DRV_SDCARD_TimerCallback);
                        dObj->timerFlag = true;
                    }
        ...
        case DRV_SDCARD_TASK_READ_DATA:
            {
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, lObj->buffer, _DRV_SDCARD_MEDIA_BLOCK_SIZE, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_READ_CRC_BYTES;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_READ_CRC_BYTES:
            {
                /* Read 2 bytes of CRC data. In SPI mode of operation the CRC
                 * data is ignored. */
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, dObj->pCmdResp, 2, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_READ_COMPLETE_CHECK;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_READ_COMPLETE_CHECK:
            {
                ...
                {
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SEND_DUMMY_CLOCK_PULSES;
                }
                break;
            }

        case DRV_SDCARD_TASK_SEND_DUMMY_CLOCK_PULSES:
            {
                /* Send 8 clock pulses */
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, dObj->pCmdResp, 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_PROCESS_NEXT;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else 
                {
        ...
        case DRV_SDCARD_TASK_PROCESS_WRITE:
            {
                ...
                if (dObj->cmdState == DRV_SDCARD_CMD_EXEC_IS_COMPLETE)
                {
                    if (dObj->cmdResponse.response1.byte == 0x00)
                    {
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_WRITE_START_TOKEN;
                    }
                    else
                    {
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_READ_WRITE_ABORT;
        ...
        case DRV_SDCARD_TASK_WRITE_START_TOKEN:
            {
                ...
                DRV_SPI_BufferAddWrite2 (dObj->spiClientHandle, dObj->pCmdResp, 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_DATA;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else 
                {
        ...
        case DRV_SDCARD_TASK_WRITE_DATA:
            {
                DRV_SPI_BufferAddWrite2 (dObj->spiClientHandle, lObj->buffer, 512, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_CRC_BYTES;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_CRC_BYTES:
            {
                /* Send 16-bit dummy CRC for the data block that was just sent. */
                DRV_SPI_BufferAddWrite2 (dObj->spiClientHandle, dObj->pClkPulseData, 2, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_READ_RESP_TOKEN;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_READ_RESP_TOKEN:
            {
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, dObj->pCmdResp , 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_RESP_TOKEN_STATUS;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_RESP_TOKEN_STATUS:
            {
                /* Read response token byte from media, mask out top three
                 * don't care bits, and check if there was an error */
                if ((dObj->pCmdResp[0] & DRV_SDCARD_WRITE_RESPONSE_TOKEN_MASK) != DRV_SDCARD_DATA_ACCEPTED)
                {
                ...
                {
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_WRITE_CHECK_BUSY;
                    dObj->timerFlag = false;
                }
                break;
            }

        case DRV_SDCARD_TASK_WRITE_CHECK_BUSY:
            {
                ...
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_CHECK_BUSY_STATUS;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_CHECK_BUSY_STATUS:
            {
                if (dObj->pCmdResp[0] == 0x00)
                {
                    ...
                    {
                        dObj->timerState = false;
                        dObj->timerHandle = SYS_TMR_CallbackSingle(_DRV_SDCARD_WRITE_TIMEOUT_IN_MS,
                                (uintptr_t)&dObj->timerState,
                                DRV_SDCARD_TimerCallback);
                        dObj->timerFlag = true;
                    }
                ...
                else
                {
                    ...
                    SYS_TMR_CallbackStop (dObj->timerHandle);
                    dObj->timerFlag = false;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_WRITE_COMPLETE_CHECK;
                }

                break;
            }

        case DRV_SDCARD_TASK_WRITE_COMPLETE_CHECK:
            {
                ...
                if (lObj->command == DRV_SDCARD_WRITE_MULTI_BLOCK)
                {
                    ...
                    if (lObj->sectorCount == 0)
                    {
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_WRITE_STOP_TRAN_TOKEN;
                    }
                    else
                    {
                        lObj->buffer += _DRV_SDCARD_MEDIA_BLOCK_SIZE;
        ...
        case DRV_SDCARD_TASK_WRITE_STOP_TRAN_TOKEN:
            {
                dObj->pCmdResp[0] = DRV_SDCARD_DATA_STOP_TRAN_TOKEN;
                DRV_SPI_BufferAddWrite2 (dObj->spiClientHandle, dObj->pCmdResp, 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_STOP_TRAN_DUMMY_PULSES;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                    dObj->timerFlag = false;
                }
                else
        ...
        case DRV_SDCARD_TASK_WRITE_STOP_TRAN_DUMMY_PULSES:
            {
                DRV_SPI_BufferAddRead2 (dObj->spiClientHandle, dObj->pCmdResp, 1, 0, dObj, &dObj->spiBufferHandle);
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_STOP_TRAN_CHECK_BUSY;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_STOP_TRAN_CHECK_BUSY:
            {
                /* The media will now send busy token (0x00) bytes until it is
                 * internally ready again (after the block is successfully
                 * written and the card is ready to accept a new block). */
                ...
                if (dObj->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
                {
                    dObj->nextTaskState = DRV_SDCARD_TASK_WRITE_STOP_TRAN_BUSY_STATUS;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_SPI_STATUS;
                }
                else
                {
        ...
        case DRV_SDCARD_TASK_WRITE_STOP_TRAN_BUSY_STATUS:
            {
                if (dObj->pCmdResp[0] == 0x00)
                {
                    ...
                    {
                        dObj->taskBufferIOState = DRV_SDCARD_TASK_WRITE_STOP_TRAN_CHECK_BUSY;
                    }
                }
                else
                {
                    ...
                    SYS_TMR_CallbackStop (dObj->timerHandle);
                    dObj->timerFlag = false;
                    dObj->taskBufferIOState = DRV_SDCARD_TASK_PROCESS_NEXT;
                }

                break;
            }

        case DRV_SDCARD_TASK_READ_WRITE_ABORT:
            {
                /* Reset any of the error flags. */
            }
        case DRV_SDCARD_TASK_PROCESS_NEXT:
            {
                /* De select the chip */
                _DRV_SDCARD_CHIP_DESELECT (dObj->chipSelectPort, dObj->chipSelectBitPosition);

                if (dObj->taskBufferIOState == DRV_SDCARD_TASK_PROCESS_NEXT)
                {
