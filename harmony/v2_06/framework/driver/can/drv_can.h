/*******************************************************************************
  ...
  Description:
    The CAN device driver provides a simple interface to manage the CAN module
    on Microchip microcontrollers. This file defines the interface Declarations
    for the CAN driver.
...
/* Function:
  ...
  Parameters:
    SYS_MODULE_INDEX index    - Index for the driver instance to be initialized

    SYS_MODULE_INIT *init     - Pointer to a data structure containing any data
            necessary to initialize the driver. This pointer may be null if no
            data is required because static overrides have been provided.
  ...
  Returns:
    ...
    argument to DRV_SAMPLE_Reinitialize, DRV_SAMPLE_Deinitialize,
    DRV_SAMPLE_Tasks and DRV_SAMPLE_Status functions.
...
/* Function:
  ...
  Parameters:
    SYS_MODULE_INDEX    object  - returned from a DRV_CAN_Initialize call
...
/* Function:
  ...
  Parameters:
    drvIndex -for static implementation pass in DRV_CAN_INDEX_X
    intent
...
// *****************************************************************************
/* Function:
     bool DRV_CAN_ChannelMessageTransmit(DRV_HANDLE handle,
                                        CAN_CHANNEL channelNum, int address,
  ...
  Description:
    ...
    address, and data length given. If the address uses more than 11 bits
    the format is changed from standard to extended to support up to 29 bits.

  Precondition:
    DRV_CAN_Initialize has been called.

  Parameters:
    DRV_HANDLE  handle  A valid open-instance handle, returned from the driver's
                   open function
  ...
  Remarks:
    This routine transmits a standard or extended message. It cannot send a
    standard (11 bits) message as extended (29 bits)
*/
...
// *****************************************************************************
/* Function:
     bool DRV_CAN_ChannelMessageReceive(DRV_HANDLE handle,
                                        CAN_CHANNEL channelNum, int address,
                                        uint8_t DLC, uint8_t* message);
  ...
  Returns:
    ...
    Boolean "false" - When a message has not been received

  Remarks:
    This routine receives a standard or extended message based upon the CAN
    Driver setup.
*/
