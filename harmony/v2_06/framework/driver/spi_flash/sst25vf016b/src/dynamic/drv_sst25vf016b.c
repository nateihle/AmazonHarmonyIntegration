/*******************************************************************************
  SST25VF016B SPI Flash Driver Dynamic implemention.
...
DRV_SST25VF016B_OBJ gDrvSST25VF016BObj[DRV_SST25VF016B_INSTANCES_NUMBER] ;

/* This is the client object array. */
DRV_SST25VF016B_CLIENT_OBJ gDrvSST25VF016BClientObj[DRV_SST25VF016B_CLIENTS_NUMBER];

/* This is the array of SST25VF016B SPI Flash Driver Buffer objects. */
DRV_SST25VF016B_BUFFER_OBJ gDrvSST25VF016BBufferObj[DRV_SST25VF016B_QUEUE_DEPTH_COMBINED];

/* This object maintains data that is required by all SST25VF016B
   driver instances. */
DRV_SST25VF016B_COMMON_DATA_OBJ gDrvSST25VF016BCommonDataObj;

// *****************************************************************************
// *****************************************************************************
// Section: SST25VF016B SPI Flash Driver Interface Implementations
// *****************************************************************************
// *****************************************************************************

// *****************************************************************************
/* Function:
    DRV_SST25VF016B_CLIENT_OBJ * _DRV_SST25VF016B_DriverHandleValidate(DRV_HANDLE handle)

  Summary:
    Dynamic implementation of the _DRV_SST25VF016B_DriverHandleValidate() function.

  Description:
    Dynamic implementation of the _DRV_SST25VF016B_DriverHandleValidate() function.
    This function returns NULL if the handle is invalid else it return a pointer
    to the SST25VF016B Driver Client Object associated with this handle.
...
DRV_SST25VF016B_CLIENT_OBJ * _DRV_SST25VF016B_DriverHandleValidate(DRV_HANDLE handle)
{
    ...
    DRV_SST25VF016B_CLIENT_OBJ * client;
    ...
    client = (DRV_SST25VF016B_CLIENT_OBJ *)handle;
...
/* Function:
  ...
  Summary:
    Dynamic impementation of DRV_SST25VF016B_Initialize system interface function.

  Description:
    This is the dynamic impementation of DRV_SST25VF016B_Initialize system
    interface function.
  
  Remarks:
    See drv_sst25vf016b.h for usage information.
*/
...
{
    DRV_SST25VF016B_OBJ *dObj = (DRV_SST25VF016B_OBJ*)NULL;
    DRV_SST25VF016B_INIT *sst25vf016bInit = NULL ;

    /* Check if the specified driver index is in valid range */
    if(drvIndex >= DRV_SST25VF016B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid driver index \n");
        return SYS_MODULE_OBJ_INVALID;
    }

    /* Check if this hardware instance was already initialized */
    if(gDrvSST25VF016BObj[drvIndex].inUse != false)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Instance already in use \n");
        return SYS_MODULE_OBJ_INVALID;
    }
    
    /* Assign to the local pointer the init data passed */
    sst25vf016bInit = ( DRV_SST25VF016B_INIT * ) init ;

    /* Allocate the driver object and set the operation flag to be in use */
    dObj = &gDrvSST25VF016BObj[drvIndex];
    dObj->inUse = true;
    ...
    dObj->spiDriverModuleIndex = sst25vf016bInit->spiDriverModuleIndex;
    
    dObj->nClients = 0;
    dObj->isExclusive = false;
    
    /* Set the port remapping pins as per init structure */
    dObj->chipSelectPortChannel = sst25vf016bInit->chipSelectPortChannel;
    dObj->chipSelectBitPosition = sst25vf016bInit->chipSelectBitPosition;


    if(_DRV_SST25VF016B_HardwareHoldIsEnabled())
    {
        ...
        SYS_PORTS_PinDirectionSelect(PORTS_ID_0,SYS_PORTS_DIRECTION_OUTPUT, sst25vf016bInit->holdPortChannel, sst25vf016bInit->holdBitPosition);
        /* Keep the HOLD pin always HIGH, it should never have high to low transition */
        SYS_PORTS_PinSet(PORTS_ID_0, sst25vf016bInit->holdPortChannel, sst25vf016bInit->holdBitPosition);
    }

    if(_DRV_SST25VF016B_HardwareWPIsEnabled())
    {
        ...
        SYS_PORTS_PinDirectionSelect(PORTS_ID_0,SYS_PORTS_DIRECTION_OUTPUT, sst25vf016bInit->writeProtectPortChannel, sst25vf016bInit->writeProtectBitPosition);
        /* Keep the WP pin always HIGH so that status refister will be always writeble */
        SYS_PORTS_PinSet(PORTS_ID_0, sst25vf016bInit->writeProtectPortChannel, sst25vf016bInit->writeProtectBitPosition);
    }
    ...
    dObj->queueTail = NULL;
    dObj->queueSize = sst25vf016bInit->queueSize;
    dObj->queueOccupancy = 0;

    /* Set driver task state to opening spi driver */
    dObj->state = DRV_SST25VF016B_TASK_OPEN_SPI_DRIVER;
    
    /* Set internal operations state */
    dObj->bufferProcessState = DRV_SST25VF016B_BUFFER_PROCESS_INIT;
...
//       then create these. */
//
//     if(!gDrvSST25VF016BCommonDataObj.membersAreInitialized)
//     {
//         /* This means that mutexes where not created. Create them. */
//         OSAL_ASSERT((OSAL_MUTEX_Create(&(gDrvSST25VF016BCommonDataObj.mutexClientObjects)) == OSAL_RESULT_TRUE),
//                     "Unable to create client instance mutex");
//         OSAL_ASSERT((OSAL_MUTEX_Create(&(gDrvSST25VF016BCommonDataObj.mutexBufferQueueObjects)) == OSAL_RESULT_TRUE),
//                     "Unable to create buffer queue objects mutex");
//
//         /* Set this flag so that global mutexes get allocated only once */
//         gDrvSST25VF016BCommonDataObj.membersAreInitialized = true;
//     }
...
void DRV_SST25VF016B_Deinitialize( SYS_MODULE_OBJ object)
{
    DRV_SST25VF016B_OBJ * dObj;
    DRV_SST25VF016B_BUFFER_OBJ * iterator;


    /* Check that the object is valid */
    if(object == SYS_MODULE_OBJ_INVALID)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid system module object \n" );
        return;
    }
    
    if(object >= DRV_SST25VF016B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid system module object \n" );
        return;
    }

    dObj = (DRV_SST25VF016B_OBJ*) &gDrvSST25VF016BObj[object];
    ...
    /* Deinitialize the SST25VF016B status */
...
SYS_STATUS DRV_SST25VF016B_Status( SYS_MODULE_OBJ object)
{
    ...
    if(object == SYS_MODULE_OBJ_INVALID)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid system object handle \n");
    ...
    return (gDrvSST25VF016BObj[object].status);
}
...
)
{
    ...
    if (drvIndex >= DRV_SST25VF016B_INSTANCES_NUMBER)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid Driver Instance \n");
    ...
    {
        /* The SST25VF016B module should be ready */

        SYS_DEBUG(0, "DRV_SST25VF016B: Was the driver initialized? \n");
        return DRV_HANDLE_INVALID;
    }
    ...
    if(dObj->isExclusive)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Driver already opened exclusively \n"); 
        return ( DRV_HANDLE_INVALID ) ;
    }
    ...
    if((dObj->nClients > 0) && (ioIntent & DRV_IO_INTENT_EXCLUSIVE))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Driver already opened. Cannot be opened exclusively \n");
        return(DRV_HANDLE_INVALID);
    }

    /* Grab client object mutex here */

 //   if(OSAL_MUTEX_Lock(&(gDrvSST25VF016BCommonDataObj.mutexClientObjects), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
    {
        ...
        for(iClient = 0; iClient != DRV_SST25VF016B_CLIENTS_NUMBER; iClient ++)
        {
            if(!gDrvSST25VF016BClientObj[iClient].inUse)
            {
                /* This means we have a free client object to use */
                clientObj = &gDrvSST25VF016BClientObj[iClient];
                ...
                clientObj->clientStatus = DRV_SST25VF016B_CLIENT_STATUS_READY;
...
void DRV_SST25VF016B_Close( const DRV_HANDLE handle)
{
    ...
    DRV_SST25VF016B_CLIENT_OBJ  * clientObj;
    DRV_SST25VF016B_OBJ * dObj;
    DRV_SST25VF016B_BUFFER_OBJ * iterator = NULL;

    /* Validate the handle */
    clientObj = _DRV_SST25VF016B_DriverHandleValidate(handle);

    if(clientObj == NULL)
    {
        /* Driver handle is not valid */
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid Driver Handle \n");
        return;
    }

    dObj = (DRV_SST25VF016B_OBJ *)clientObj->hDriver;
 ...
 //   {
        ...
        while(iterator != NULL)
        {
            if(clientObj == (DRV_SST25VF016B_CLIENT_OBJ *)iterator->hClient)
            {
    ...
    // {
        ...
        // SYS_DEBUG(0, "DRV_SST25VF016B: Could not remove client buffer objects \n");
        // clientObj->clientStatus = DRV_SST25VF016B_CLIENT_STATUS_ERROR;
        // return;
    // }
    ...
    clientObj->clientStatus = DRV_SST25VF016B_CLIENT_STATUS_CLOSED;
...
/* Function:
  ...
  Summary:
    Dynamic impementation of DRV_SST25VF016B_ClientStatus client interface function.

  Description:
    This is the dynamic impementation of DRV_SST25VF016B_ClientStatus client interface 
    function.
...
DRV_SST25VF016B_CLIENT_STATUS DRV_SST25VF016B_ClientStatus(DRV_HANDLE handle)
{
    ...
    client = _DRV_SST25VF016B_DriverHandleValidate(handle);
    ...
    if(client == NULL)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid driver handle \n");
        return DRV_SST25VF016B_CLIENT_STATUS_CLOSED;
    }
...
SYS_FS_MEDIA_GEOMETRY * DRV_SST25VF016B_GeometryGet( DRV_HANDLE handle )
{
    ...
    DRV_SST25VF016B_CLIENT_OBJ * hClient;
    DRV_SST25VF016B_OBJ *hDriver;

    /* Validate the driver handle */
    hClient = _DRV_SST25VF016B_DriverHandleValidate(handle);
...
// *****************************************************************************
/* Function:
    void _DRV_SST25VF016B_ProcessBufferTask
    (
        DRV_SST25VF016B_OBJ * hDriver,
        DRV_SST25VF016B_BUFFER_OBJ * bufferObj
    )
  ...
  Description:
    Dynamic implementation of the _DRV_SST25VF016B_ProcessBufferTask() function.
    ...
    switch(hDriver->bufferProcessState)
    {
        case DRV_SST25VF016B_BUFFER_PROCESS_INIT:
        {
            ...
            if (bufferObj->operation == DRV_SST25VF016B_BLOCK_READ)
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_SEND_READ_CMD_AND_ADDRESS;
            }
            else
            {
                /* If request is for Erase/Write, then load Write Enable command first */
                hDriver->commandAddressData[0] = SST25VF016B_WREN_OP_CODE;

                hDriver->bufferProcessState = DRV_SST25VF016B_SEND_WREN_CMD;
                break;
            }       
        }
         case DRV_SST25VF016B_SEND_READ_CMD_AND_ADDRESS:
         {
                     ...
                     (uint8_t *)&hDriver->commandAddressData[0], 4, NULL, NULL)) != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_START_READING;
            }
            else
            {
        ...
        case DRV_SST25VF016B_START_READING:
        {
            ...
            {
                hDriver->bufferProcessState = DRV_SST25VF016B_READ_COMPLETED;
            }
            else
            {
                break;
        ...
        case DRV_SST25VF016B_READ_COMPLETED:
        {
            ...
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_BUFFER_PROCESS_INIT;
            }
            break;
        }
        case DRV_SST25VF016B_SEND_WREN_CMD:
        {
            /* Add the buffer in SPI Queue */
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_WREN_EXECUTION_STATUS_CHECK;
            }
            else
            {
                /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF016B_WREN_EXECUTION_STATUS_CHECK:
        {
            /* Check if the transfer status is success or not */
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                if (bufferObj->operation == DRV_SST25VF016B_BLOCK_WRITE)
                {
                    /* It is a WRITE request */
                    hDriver->bufferProcessState = DRV_SST25VF016B_SEND_WRITE_CMD_ADDRESS_AND_DATA;
                }
                else
                {
                    /* This means it is an Erase request */
                    hDriver->bufferProcessState = DRV_SST25VF016B_SEND_ERASE_CMD_AND_ADDRESS;
                    break;
                }
            }
            else
        ...
        case DRV_SST25VF016B_SEND_WRITE_CMD_ADDRESS_AND_DATA:
        {
            /* Enable CS Line */
            SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

            /* Add Write Byte op code */
            hDriver->commandAddressData[0] = SST25VF016B_WRITE_BYTE_OP_CODE;
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_WAIT_FOR_WRITE_OR_ERASE_BUFFER_COMPLETE;
            }
            break;
        }
        case DRV_SST25VF016B_SEND_ERASE_CMD_AND_ADDRESS:
        {
            /* Enable CS Line */
            SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

            /* Add Erase op code */
            hDriver->commandAddressData[0] = SST25VF016B_ERASE_OP_CODE;
            /* Add erase address */
            hDriver->commandAddressData[1] = (uint8_t)(bufferObj->address >> 16);
            hDriver->commandAddressData[2] = (uint8_t)(bufferObj->address >> 8);
            hDriver->commandAddressData[3] = (uint8_t)(bufferObj->address);
        ...
        case DRV_SST25VF016B_WAIT_FOR_WRITE_OR_ERASE_BUFFER_COMPLETE:
        {
            /* Check if the transfer status is success or not */
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                hDriver->bufferProcessState = DRV_SST25VF016B_SEND_COMMAND_FOR_BUSY_STATUS;
            }
            else
            {
                break;
        ...
        case DRV_SST25VF016B_SEND_COMMAND_FOR_BUSY_STATUS:
        {
            /* Enable CS Line */
            SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

            /* Add RDSR op code to read the status register */
            hDriver->commandAddressData[0] = SST25VF016B_RDSR_OP_CODE;
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                /* This means command has been allocated in the SPI queue,
                 *  now wait for it to be executed in the next state */
                hDriver->bufferProcessState = DRV_SST25VF016B_READ_BUSY_STATUS;
            }
            else
            {
                /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF016B_READ_BUSY_STATUS:
        {
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                /* This means command has been allocated in the SPI queue,
                 *  now wait for it to be executed in the next state */
                hDriver->bufferProcessState = DRV_SST25VF016B_WAIT_FOR_BUSY_CLEAR;
            }
            else
            {
                /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF016B_WAIT_FOR_BUSY_CLEAR:
        {
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
...
void DRV_SST25VF016B_Tasks ( SYS_MODULE_OBJ object )
{
    ...
    if(object >= DRV_SST25VF016B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid module object \n");
        return;
    }

    hDriver = (DRV_SST25VF016B_OBJ*) &gDrvSST25VF016BObj[object];
    ...
    {
        case DRV_SST25VF016B_TASK_OPEN_SPI_DRIVER:
        {
            ...
            if(hDriver->spiDriverOpenHandle != DRV_HANDLE_INVALID)
            {
                ...
                * WRITE Protection of SST Flash in the next state */
                hDriver->state = DRV_SST25VF016B_SEND_WREN_CMD_WRSR;
            }
            else
            {
        ...
        case DRV_SST25VF016B_SEND_WREN_CMD_WRSR:
        {
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                /* If the command has been queued,
                 * then check if it has been processed in the next state */
                hDriver->state = DRV_SST25VF016B_WREN_EXECUTION_STATUS_CHECK_WRSR;
            }
            else
            {
                /* this means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF016B_SEND_WRSR_CMD_AND_VALUE:
        {
            ...
            /* Add WRSR op code */
            hDriver->commandAddressData[0] = SST25VF016B_WRSR_OP_CODE;
            ...
            if (hDriver->spiBufferHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
                /* If the command has been queued,
                 * then check if it has been processed in the next state */
                hDriver->state = DRV_SST25VF016B_WRSR_EXECUTION_STATUS_CHECK;
            }
            else
            {
        ...
        case DRV_SST25VF016B_WRSR_EXECUTION_STATUS_CHECK:
        {
            /* Check if the transfer status is success or not */
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                hDriver->state = DRV_SST25VF016B_TASK_PROCESS_QUEUE;
            }
            else
            {
        ...
        case DRV_SST25VF016B_TASK_PROCESS_QUEUE:
        {
            ...
            if(bufferObj != NULL)
            {
                ...
                if(bufferObj->nCurrentBlocks >= bufferObj->size)
                {
                    ...
                    client = (DRV_SST25VF016B_CLIENT_OBJ *)bufferObj->hClient;
                    if(client->eventHandler != NULL)
                    {
                        client->eventHandler(DRV_SST25VF016B_EVENT_BLOCK_COMMAND_COMPLETE,
                            (DRV_SST25VF016B_BLOCK_COMMAND_HANDLE)bufferObj,
                            client->context);
                    }
            ...
            {
                _DRV_SST25VF016B_ProcessBufferTask(hDriver, hDriver->queueTail);
            }
            break;
        }
        default:
...
/* Function:
  ...
  Description:
    ...
    The function returns DRV_SST25VF016B_EVENT_BLOCK_HANDLE_INVALID in the
    ...
    driver will issue a DRV_SST25VF016B_EVENT_ERASE_COMPLETE event if the
    erase operation was successfull or DRV_SST25VF016B_EVENT_ERASE_ERROR
    event if the erase operation was not successfull.

    The block start address should be aligned on a
    DRV_SST25VF016B_ERASE_BLOCK_SIZE byte boundary.
...
{
    DRV_SST25VF016B_CLIENT_OBJ * clientObj = NULL;
    DRV_SST25VF016B_OBJ * hDriver = NULL;
    DRV_SST25VF016B_BUFFER_OBJ * bufferObj = NULL, * iterator = NULL;
    unsigned int i = 0;
    ...
    {
        *commandHandle = DRV_SST25VF016B_BLOCK_COMMAND_HANDLE_INVALID;
    }

    /* Validate the driver handle */
    clientObj = _DRV_SST25VF016B_DriverHandleValidate(hClient);
    if(clientObj == NULL)
    {
    ...
    if(!(clientObj->ioIntent & DRV_IO_INTENT_WRITE))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Driver is not open in Write mode \n");
        return;
    }
    ...
    if((nBlock == 0))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid block size parameter \n");
        return;
    }
    ...
    if(hDriver->queueOccupancy >= hDriver->queueSize)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Queue is full for this driver instance \n");
        return;
    }

    /* Search the buffer pool for a free buffer object */
    for(i = 0 ; i < DRV_SST25VF016B_QUEUE_DEPTH_COMBINED; i ++)
    {
        if(!gDrvSST25VF016BBufferObj[i].inUse)
        {
            /* This means this object is free.
             * Configure the object and then
             * break */
            bufferObj = &gDrvSST25VF016BBufferObj[i];
            ...
            bufferObj->operation = DRV_SST25VF016B_BLOCK_ERASE;

            if(commandHandle != NULL)
            {
                /* Assign a handle to this buffer */
                *commandHandle = (DRV_SST25VF016B_BLOCK_COMMAND_HANDLE)bufferObj;
                break;
            }
        }
    }

    if(i == DRV_SST25VF016B_QUEUE_DEPTH_COMBINED)
    {
        /* This means we could not find a buffer. This
           will happen if the the DRV_SST25VF016B_QUEUE_DEPTH_COMBINED
           parameter is configured to be less */

        SYS_DEBUG(0, "DRV_SST25VF016B: Insufficient Queue Depth");
        return;
    }
...
/* Function:
  ...
  Description:
    ...
    returns DRV_SST25VF016B_EVENT_BLOCK_HANDLE_INVALID in the commandHandle
    ...
    - if the driver handle is invalid
    If the requesting client registered an event callback with the driver, the
    driver will issue a DRV_SST25VF016B_EVENT_BLOCK_COMPLETE event if the
    buffer was processed successfully of DRV_SST25VF016B_EVENT_BLOCK_ERROR
    event if the buffer was not processed successfully.

    The block start address should be aligned on a
    DRV_SST25VF016B_READ_BLOCK_SIZE byte boundary.
...
)
{
    ...
    if(!(clientObj->ioIntent & DRV_IO_INTENT_READ))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Driver is not open in Read mode \n");
        return;
    }
    ...
    if((nBlock == 0) || (targetBuffer == NULL ))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Invalid parameters \n");
        return;
    }
    ...
    for(i = 0 ; i < DRV_SST25VF016B_QUEUE_DEPTH_COMBINED; i ++)
    {
        if(!gDrvSST25VF016BBufferObj[i].inUse)
        {
            ...
            bufferObj->operation = DRV_SST25VF016B_BLOCK_READ;
...
/* Function:
  ...
  Description:
    ...
    If the requesting client registered an event callback with the driver, the
    driver will issue a DRV_SST25VF016B_EVENT_BLOCK_COMPLETE event if the
    buffer was processed successfully or DRV_SST25VF016B_EVENT_BLOCK_ERROR
    event if the buffer was not processed successfully.

    The block start address should be aligned on a
    DRV_SST25VF016B_WRITE_BLOCK_SIZE byte boundary.
...
)
{
    ...
    if(hDriver->queueOccupancy >= hDriver->queueSize)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF016B: Queue is full \n");
        return;
    }
    ...
    for(i = 0 ; i < DRV_SST25VF016B_QUEUE_DEPTH_COMBINED; i ++)
    {
        if(!gDrvSST25VF016BBufferObj[i].inUse)
        {
            ...
            bufferObj->operation = DRV_SST25VF016B_BLOCK_WRITE;
