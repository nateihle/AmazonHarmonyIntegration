...
DRV_SAMPLE_CLIENT_OBJ       gDrvSAMPLEClientObj[DRV_SAMPLE_CLIENTS_NUMBER];
...
DRV_SAMPLE_CLIENT_OBJ _DRV_SAMPLE_ClientObjectAllocate( SYS_MODULE_INDEX drvIndex )
{
    /* Local Variables */
    DRV_SAMPLE_CLIENT_OBJ_HANDLE hClientObj = ( DRV_SAMPLE_CLIENT_OBJ_HANDLE )0;
    uint8_t i = 0;
...
} /* _DRV_SAMPLE_ClientObjectAllocate */
...
void _DRV_SAMPLE_MAKE_NAME( Initialize )( const SYS_MODULE_INIT * const init )
{
    ...
    _DRV_SAMPLE_OBJ( DRV_SAMPLE_INDEX, numClients ) = 0;
...
/* Function:
  ...
  Description:
    ...
    the SAMPLE module using the hardware initialization given data. It does not
...
*/
 ...
 DRV_HANDLE DRV_SAMPLE_Open( const DRV_IO_INTENT ioIntent )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SAMPLE_CLIENT_OBJ_HANDLE hClientObj = (DRV_SAMPLE_CLIENT_OBJ_HANDLE) 0;
    DRV_SAMPLE_OBJ_HANDLE hObj = 0;
    ...
    hClientObj = _DRV_SAMPLE_ClientObjectAllocate( _DRV_SAMPLE_INDEX_GET( DRV_SAMPLE_INDEX ) );
    
    gDrvSAMPLEClientObj[hClientObj].inUse = true;
    gDrvSAMPLEClientObj[hClientObj].driverObject = _DRV_SAMPLE_INDEX_GET( DRV_SAMPLE_INDEX );

    /* Increment the client in case of Multi client support, otherwise remove
       the below statement */
    hObj = gDrvSAMPLEClientObj[hClientObj].driverObject;

    /* Check for exclusive access */
    if( ( gDrvSAMPLEObj[dObj].IsExclusive == true ) ||
        ( gDrvSAMPLEObj[dObj].numClients > 0) && DRV_IO_ISEXCLUSIVE( ioIntent ) )
    {
    ...
    if( gDrvSAMPLEObj[dObj].numClients > DRV_SAMPLE_CLIENTS_NUMBER )
    {
        /* Set that the hardware instance is opened with max clients */
        return DRV_HANDLE_INVALID;
    }

    gDrvSAMPLEObj[hObj].numClients++  ;

    /* Check for the client object */
    SYS_ASSERT( hClientObj < DRV_SAMPLE_CLIENTS_NUMBER , "Invalid Client Object" );

    /* To Do: OSAL - Unlock Mutex */

    /* Update the Client Status */
    gDrvSAMPLEClientObj[hClientObj].status = DRV_SAMPLE_CLIENT_STATUS_READY;
...
void DRV_SAMPLE_Close( DRV_HANDLE handle )
{
    ...
    gDrvSAMPLEClientObj[hClientObj].inUse = false ;

    /* To Do: OSAL - unlock Mutex */

    /* Update the Client Status */
    gDrvSAMPLEClientObj[hClientObj].status = DRV_SAMPLE_CLIENT_STATUS_INVALID;
...
DRV_SAMPLE_CLIENT_STATUS DRV_SAMPLE_ClientStatus( DRV_HANDLE handle )
{
    ...
    return( gDrvSAMPLEClientObj[hClientObj].status );
