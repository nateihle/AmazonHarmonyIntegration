...
{
    if (queueManager == (uintptr_t)NULL)
    {
        return DRV_SPI_SYS_QUEUE_INVALID_PARAMETER;
    }
...
{
    SYS_INT_PROCESSOR_STATUS intStatus;
    if ((element == NULL) || (queue == (uintptr_t)NULL))
    {
        return DRV_SPI_SYS_QUEUE_INVALID_PARAMETER;
    }
    ...
    intStatus = SYS_INT_StatusGetAndDisable();
    ...
    pQueue->numAlloc++;
	SYS_INT_StatusRestore(intStatus);
