...
sint8 chip_wake(void)
{
	...
	ret = nm_read_reg_with_ret(HOST_CORT_COMM, (uint32 *)&reg);
	...
	ret = nm_read_reg_with_ret(WAKE_CLK_REG, (uint32 *)&reg);
	if(ret != M2M_SUCCESS)goto _WAKE_EXIT;
	/* Set bit 1 */
	if(!(reg & NBIT1))
	{
	...
	do
	{
		ret = nm_read_reg_with_ret(CLOCKS_EN_REG, (uint32 *)&clk_status_reg);
		if(ret != M2M_SUCCESS) {
...
sint8 wait_for_bootrom(uint8 arg)
{
	...
	uint32 reg = 0, cnt = 0;
	uint32 u32GpReg1 = 0;
	uint32 u32DriverVerInfo = M2M_MAKE_VERSION_INFO(M2M_RELEASE_VERSION_MAJOR_NO, M2M_RELEASE_VERSION_MINOR_NO, \
		M2M_RELEASE_VERSION_PATCH_NO, M2M_RELEASE_VERSION_MAJOR_NO, M2M_RELEASE_VERSION_MINOR_NO, M2M_RELEASE_VERSION_PATCH_NO);
