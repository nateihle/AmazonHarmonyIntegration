...
static tstrM2mIpPktBuf s_rxFrame;
...
static void m2m_wifi_cb(uint8 u8OpCode, uint16 u16DataSize, uint32 u32Addr)
{
	...
	else if(u8OpCode == M2M_WIFI_RESP_ETHERNET_RX_PACKET)
	{
        ...
        if(hif_receive(u32Addr, (uint8 *)&strM2mRsvd, sizeof(tstrM2mIpRsvdPkt), 0) == M2M_SUCCESS) {
            ...
            WDRV_ASSERT(strM2mRsvd.u16PktSz <= s_rxFrame.u16BufSz, "Insufficient rx frame buffer");
            if (hif_receive(u32Addr + u16Offset, s_rxFrame.buffer, strM2mRsvd.u16PktSz, 1) != M2M_SUCCESS) {
                WDRV_ASSERT(false, "Error in reading a rx frame");
            }
            if (gpfAppWifiCb) {
                s_rxFrame.u16BufSz = strM2mRsvd.u16PktSz;
                gpfAppWifiCb(M2M_WIFI_RESP_ETHERNET_RX_PACKET, &s_rxFrame);
            }
        } else {
            WDRV_ASSERT(false, "Error in reading a rx frame header");
        }
	}
	else
	{
...
sint8 m2m_wifi_set_sleep_mode(uint8 PsTyp, uint8 BcastEn)
{
	...
	M2M_INFO("POWER SAVE %s\n", PsTyp ? "Enabled" : "Disabled");
...
/*!
@fn	\
	NMI_API sint8 m2m_wifi_set_receive_buffer(void *pvHeader, void *pvBuffer, uint16 u16BufferLen);
@brief
	set the ethernet receive buffer, should be called in the receive call back.
@param[in]	pvBuffer
	Pointer to the ethernet buffer header.
@param[in]	pvBuffer
...
NMI_API sint8 m2m_wifi_set_receive_buffer(void *pvHeader, void *pvBuffer, uint16 u16BufferLen)
{
    ...
    s_rxFrame.header = (uint8_t *)pvHeader;
    s_rxFrame.buffer = (uint8_t *)pvBuffer;
    s_rxFrame.u16BufSz = u16BufferLen;
    return s8ret;
}
