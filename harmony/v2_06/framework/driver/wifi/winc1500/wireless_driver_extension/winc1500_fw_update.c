/*******************************************************************************
  WINC1500 firmware update support.

  File Name:
    winc1500_fw_update.c
...
#define WRITE_BUFF              3
#define RESET                   4
#define RECONFIGURE_USART       5

#define PAYLOAD_BUFFER_SIZE     512
#define CMD_HDR_LENGTH          sizeof(cmdHdr)
...
static SYS_CONSOLE_BUFFER_DMA_READY uint16_t s_cmd_recv_buffer[8];
static SYS_CONSOLE_BUFFER_DMA_READY uint16_t s_payload_buffer[PAYLOAD_BUFFER_SIZE];
static SYS_CONSOLE_BUFFER_DMA_READY uint8_t s_tx_buffer[16];

static uint8 *s_cmd_buf;
static cmdHdr s_cmd_hdr;
...
/* In this firmware updater's use case, the following buffer size has to be a
 * multiple of PAYLOAD_BUFFER_SIZE. At the same time, it also has to be a
 * multiple of SYS_CONSOLE_USB_CDC_READ_BUFFER_SIZE, which is a USB CDC level
 * requirement. */
#define FW_UPDATE_USB_CDC_READ_BUFFER_SIZE PAYLOAD_BUFFER_SIZE

#define CONSOLE_USB_RD_BUSY (console_usb_rd_buf && (console_usb_rd_buf < usb_cdc_rd_buf || console_usb_rd_buf >= usb_cdc_rd_buf + sizeof(usb_cdc_rd_buf)))
#define CONSOLE_USB_WR_BUSY (console_usb_wr_buf)
...
static size_t usb_cdc_rd_buf_sz;
static uint8_t SYS_CONSOLE_BUFFER_DMA_READY usb_cdc_rd_buf[FW_UPDATE_USB_CDC_READ_BUFFER_SIZE];
...
static SYS_CONSOLE_STATUS console_usb_status(void)
{
    ...
    if (s_consoleUsbData.isConfigured) {
        if (s_consoleUsbData.overflowFlag)
            ...
            status = (CONSOLE_USB_RD_BUSY || CONSOLE_USB_WR_BUSY) ? SYS_CONSOLE_STATUS_BUSY : SYS_CONSOLE_STATUS_CONFIGURED;
    }
    return status;
}
...
static size_t console_usb_read(void *buf, size_t count)
{
    if (!console_usb_rd_buf) {
        ...
        WDRV_ASSERT(console_usb_rd_buf_sz <= sizeof(usb_cdc_rd_buf), "USB console read size too big");
    } else {
        if (count < console_usb_rd_buf_sz) {
            memcpy(buf, console_usb_rd_buf, count);
            console_usb_rd_buf += count;
            console_usb_rd_buf_sz -= count;
            s_consoleData.rdComplete = true;
        } else if (count == console_usb_rd_buf_sz) {
            memcpy(buf, console_usb_rd_buf, count);
            console_usb_rd_buf = NULL;
            s_consoleData.rdComplete = true;
        } else { // count > console_usb_rd_buf_sz
            memcpy(buf, console_usb_rd_buf, console_usb_rd_buf_sz);
            console_usb_rd_buf = (uint8_t *)buf + console_usb_rd_buf_sz;
            console_usb_rd_buf_sz = count - console_usb_rd_buf_sz;
        }
    }
...
)
{
    ...
    case USB_DEVICE_CDC_EVENT_READ_COMPLETE:
        ...
        usb_cdc_rd_buf_sz = ((USB_DEVICE_CDC_EVENT_DATA_READ_COMPLETE *)pData)->length;
        break;
...
static void console_usb_tasks(void)
{
    switch(s_consoleUsbData.state)
    {
        ...
        case CONSOLE_USB_CDC_STATE_READY:
            ...
            if (CONSOLE_USB_RD_BUSY && s_consoleUsbData.readTransferHandle != USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID)
                s_consoleUsbData.state = CONSOLE_USB_CDC_STATE_WAIT_FOR_READ_COMPLETE;
            else if (CONSOLE_USB_WR_BUSY && s_consoleUsbData.writeTransferHandle != USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID)
                s_consoleUsbData.state = CONSOLE_USB_CDC_STATE_WAIT_FOR_WRITE_COMPLETE;
            else if (CONSOLE_USB_RD_BUSY && s_consoleUsbData.isReadComplete && s_consoleUsbData.readTransferHandle == USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID)
                s_consoleUsbData.state = CONSOLE_USB_CDC_STATE_SCHEDULE_READ;
            else if (CONSOLE_USB_WR_BUSY && s_consoleUsbData.isWriteComplete && s_consoleUsbData.writeTransferHandle == USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID)
        ...
        case CONSOLE_USB_CDC_STATE_SCHEDULE_READ:
            ...
            if (CONSOLE_USB_RD_BUSY) {
                ...
                USB_DEVICE_CDC_Read(s_consoleUsbData.cdcInstance, &s_consoleUsbData.readTransferHandle,
                    usb_cdc_rd_buf, FW_UPDATE_USB_CDC_READ_BUFFER_SIZE);
        ...
        case CONSOLE_USB_CDC_STATE_WAIT_FOR_READ_COMPLETE:
            ...
            if (s_consoleUsbData.isReadComplete) {
                static size_t console_usb_rd_sz;
                if (console_usb_rd_buf_sz <= usb_cdc_rd_buf_sz) {
                    memcpy(console_usb_rd_buf, usb_cdc_rd_buf, console_usb_rd_buf_sz);

                    s_consoleUsbData.readTransferHandle = USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID;

                    console_usb_rd_sz += console_usb_rd_buf_sz;
                    if (s_consoleUsbData.rdCallback != NULL)
                        s_consoleUsbData.rdCallback(&console_usb_rd_sz);
                    console_usb_rd_sz = 0;

                    if (console_usb_rd_buf_sz < usb_cdc_rd_buf_sz) {
                        console_usb_rd_buf = usb_cdc_rd_buf + console_usb_rd_buf_sz;
                        console_usb_rd_buf_sz = usb_cdc_rd_buf_sz - console_usb_rd_buf_sz;
                    } else { // console_usb_rd_buf_sz == usb_cdc_rd_buf_sz
                        console_usb_rd_buf = NULL;
                    }

                    s_consoleUsbData.state = CONSOLE_USB_CDC_STATE_READY;
                } else { // console_usb_rd_buf_sz > usb_cdc_rd_buf_sz
                    memcpy(console_usb_rd_buf, usb_cdc_rd_buf, usb_cdc_rd_buf_sz);

                    s_consoleUsbData.readTransferHandle = USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID;

                    console_usb_rd_sz += usb_cdc_rd_buf_sz;

                    console_usb_rd_buf += usb_cdc_rd_buf_sz;
                    console_usb_rd_buf_sz -= usb_cdc_rd_buf_sz;
...
static void nm_usart_protocol_handler(enum nm_usart_event_type event_name)
{
    ...
    case WAIT_SYNC:
        if (event_name == USART_PKT_RECEIVED) {
            ...
            else if (s_pkt[0] == 0x12) { //USART identification command
                s_tx_buffer[0] = 0x5B;
                s_cmd_recv_buffer[0] = 0xFF;
                console_write_data(1, s_tx_buffer);
                console_read_data(1, s_cmd_recv_buffer);
    ...
    case WAIT:
        if (event_name == USART_PKT_RECEIVED) {
            s_prot_status = COLLECT_HDR;
            s_cmd_buf = s_pkt;
            console_read_data(CMD_HDR_LENGTH - 1, NULL);
        }
        else {
            s_prot_status = WAIT_SYNC;
            s_tx_buffer[0] = 0xEA;
    ...
    case COLLECT_HDR:
        if (event_name == USART_PKT_RECEIVED) {
            ...
            if (checksum != 0) {
                ...
                memcpy(&s_cmd_hdr, s_cmd_buf, CMD_HDR_LENGTH);
                //Process command
                if ((s_cmd_hdr.cmd & 0xFF) == WRITE_BUFF) {
                    s_prot_status = COLLECT_PAYLOAD;
                    payload_length = (s_cmd_hdr.cmd >> 16) & 0xFFFF;
                    s_tx_buffer[0] = 0xAC;
                    console_write_data(1, s_tx_buffer);
                    console_read_data(payload_length, (uint8_t *)s_payload_buffer);
                }
                else if ((s_cmd_hdr.cmd & 0xFF) == WRITE_REG) {
                    s_command_pending = 1;
                    s_prot_status = PROCESS;
                }
                else {
                    s_command_pending = 1;
                    s_change_state = 1;
...
static sint8 enter_wifi_firmware_download(void)
{
    ...
    if (ret != M2M_SUCCESS)
    {
        M2M_ERR("Failed to put the Wi-Fi chip in download mode\n");
        return M2M_ERR_INIT;
    }
    ...
    while (!done_programming) {
        ...
        if (s_command_pending && (s_prot_status == PROCESS)) {
            uint32_t temp;
            switch((s_cmd_hdr.cmd) & 0xFF) {
                //Forward it to SERCOM0 SPI
                case READ_REG:
                    //Translate it to SPI Read register
                    temp = nm_read_reg(s_cmd_hdr.addr);
                    ...
                    break;
                case WRITE_REG:
                    //Translate it to SPI Write register
                    nm_write_reg(s_cmd_hdr.addr, s_cmd_hdr.val);
                    s_tx_buffer[0] = 0xAC;
                    s_prot_status = WAIT_SYNC;
                    console_write_data(1, s_tx_buffer);
                    console_read_data(1, s_cmd_recv_buffer);
                    break;
                case READ_BUFF:
                    //Translate it to SPI Read buffer
                    nm_read_block(s_cmd_hdr.addr, (uint8 *)&s_payload_buffer[0], ((s_cmd_hdr.cmd >> 16) & 0xFFFF));
                    s_prot_status = WAIT_SYNC;
                    console_write_data(((s_cmd_hdr.cmd >> 16) & 0xFFFF), (uint8_t *)s_payload_buffer);
                    console_read_data(1, s_cmd_recv_buffer);
                    break;
                case WRITE_BUFF:
                    //Translate it to SPI Write buffer
                    nm_write_block(s_cmd_hdr.addr, (uint8 *)&s_payload_buffer[0], ((s_cmd_hdr.cmd >> 16) & 0xFFFF));
                    s_tx_buffer[0] = 0xAC;
                    s_prot_status = WAIT_SYNC;
                    console_write_data(1, s_tx_buffer);
                    console_read_data(1, s_cmd_recv_buffer);
                    break;
                case RECONFIGURE_USART:
                    //Send the ACK back
                    s_prot_status = WAIT_SYNC;
                    s_reconfig = 1;
                    console_read_data(1, s_cmd_recv_buffer);
