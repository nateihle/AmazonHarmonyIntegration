/*******************************************************************************
  MRF24WN Wireless Driver Configuration

  File Name:
    wdrv_mrf24wn_config.c

  Summary:
    Manage MRF24WN configuration.
...
static void SaveConfigToMemory(void)
{
    ...
    if (WDRV_NVM_PageEraseConfigWrite((uint8_t *)gp_wdrv_cfg, sizeof(WDRV_CONFIG), WDRV_NVM_SPACE_ADDR))
        WDRV_DBG_INFORM_PRINT("MRF24WN: NVM operation succeeded\r\n");
    else
        WDRV_DBG_INFORM_PRINT("MRF24WN: NVM operation failed\r\n");
}
...
static bool LoadConfigFromMemory(void)
{
    if (s_wdrv_cfgBackup.verifyFlag == WDRV_NVM_SPACE_MAGIC_NUMBER) {
        /*LDRA_INSPECTED 480 S*/ /*Deviation Reference: MH-6780*/
...
{
    gp_wdrv_cfg->verifyFlag = WDRV_NVM_SPACE_MAGIC_NUMBER;
    /*LDRA_INSPECTED 480 S*/ /*Deviation Reference: MH-6782*/
    memcpy((uint8_t *)&s_wdrv_cfgBackup, (uint8_t *)gp_wdrv_cfg, sizeof(WDRV_CONFIG));
    WDRV_DBG_INFORM_PRINT("MRF24WN: Save operation succeeded\r\n");
}

static void DeleteConfigInMemory(void)
{
    memset(&s_wdrv_cfgBackup, 0, sizeof(WDRV_CONFIG));
    WDRV_DBG_INFORM_PRINT("MRF24WN: Erase operation succeeded\r\n");
}
...
bool WDRV_CONFIG_Load(void)
{
    ...
    if (!res) {
        ...
        memcpy(gp_wdrv_cfg->ssid, (const void *)WDRV_DEFAULT_SSID, sizeof(WDRV_DEFAULT_SSID)); // including '\0'
        ...
        case WDRV_SECURITY_WEP_40:
            if (sizeof(WDRV_DEFAULT_WEP_KEYS_40) - 1 == 5 || sizeof(WDRV_DEFAULT_WEP_KEYS_40) - 1 == 10) {
                memcpy(gp_wdrv_cfg->securityKey, (const void *)WDRV_DEFAULT_WEP_KEYS_40, sizeof(WDRV_DEFAULT_WEP_KEYS_40)); // including '\0'
        ...
        case WDRV_SECURITY_WEP_104:
            if (sizeof(WDRV_DEFAULT_WEP_KEYS_104) - 1 == 13 || sizeof(WDRV_DEFAULT_WEP_KEYS_104) - 1 == 26) {
                memcpy(gp_wdrv_cfg->securityKey, (const void *)WDRV_DEFAULT_WEP_KEYS_104, sizeof(WDRV_DEFAULT_WEP_KEYS_104)); // including '\0'
        ...
        case WDRV_SECURITY_WPA2_WITH_PASS_PHRASE:
            if (sizeof(WDRV_DEFAULT_PSK_PHRASE) - 1 >= WDRV_MIN_WPA_PASS_PHRASE_LENGTH && sizeof(WDRV_DEFAULT_PSK_PHRASE) - 1 <= WDRV_MAX_WPA_PASS_PHRASE_LENGTH) {
                memcpy(gp_wdrv_cfg->securityKey, (const void *)WDRV_DEFAULT_PSK_PHRASE, sizeof(WDRV_DEFAULT_PSK_PHRASE)); // including '\0'
        ...
        case WDRV_SECURITY_WPS_PIN:
            if (sizeof(WDRV_DEFAULT_WPS_PIN) - 1 == WDRV_WPS_PIN_LENGTH) {
                memcpy(gp_wdrv_cfg->securityKey, (const void *)WDRV_DEFAULT_WPS_PIN, sizeof(WDRV_DEFAULT_WPS_PIN)); // including '\0'
                gp_wdrv_cfg->securityKeyLen = sizeof(WDRV_DEFAULT_WPS_PIN) - 1;
            } else {
                memset(gp_wdrv_cfg->securityKey, 0x00, sizeof(gp_wdrv_cfg->securityKey));
                gp_wdrv_cfg->securityKeyLen = 0;
            }
