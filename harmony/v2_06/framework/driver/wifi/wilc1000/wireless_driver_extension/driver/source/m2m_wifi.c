...
#include "driver/wifi/wilc1000/wireless_driver_extension/driver/source/nmdrv.h"
...
#if 0 //TBR: Need to find out why this is not working			
...
#else
			strM2mIpCtrlBuf.u8DataOffset = pstrM2MIpRxPkt->u16PktOffset;
			gpfAppEthCb(M2M_WIFI_RESP_ETHERNET_RX_PACKET, pu8Buffer + strM2mIpCtrlBuf.u8DataOffset, &(strM2mIpCtrlBuf));
#endif
		}
	}
...
sint8 m2m_wifi_send_ethernet_pkt(uint8* pu8Packet, uint16 u16PacketSize, uint8	u8IfcId) 
{
	...
	{
#if 0	//TBR: Need to find out why hif_send_optimized() is not working
		...
		(uint8*)&strTxPkt, sizeof(tstrM2MWifiTxPacketInfo), pu8Packet, u16PacketSize,  M2M_ETHERNET_HDR_OFFSET + 2 - M2M_HIF_HDR_OFFSET);
