...
static SYS_MODULE_OBJ WDRV_WILC1000_Initialize(const SYS_MODULE_INDEX index, const SYS_MODULE_INIT *const init)
{
    ...
    memset(&(hooks.wdrvIndications), 0, sizeof(hooks.wdrvIndications));
...
void WDRV_WILC1000_Default_Hook_Initialize(WDRV_HOOKS *ehooks)
{
	memset(ehooks, 0, sizeof(WDRV_HOOKS));
	
    ehooks->frame_received = PushFrameToFifo;
    ehooks->get_rx_buf = GetAvailRxBuf;
    ehooks->RFReady = RfReadyHandler;
    ehooks->ScanDone = ScanDoneHandler;
    ehooks->ConnectEvent = ConnectEventCB;
    ehooks->WPSDone = WPSDoneCB;
    
    ehooks->isEthMode = true;
