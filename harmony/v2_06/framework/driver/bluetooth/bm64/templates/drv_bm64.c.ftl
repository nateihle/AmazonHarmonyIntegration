...
#include "driver/bluetooth/bm64/drv_bm64_command_decode.h"
#include "driver/bluetooth/bm64/drv_bm64_ble.h"
#include "driver/bluetooth/bm64/drv_bm64_uart.h"
#include "driver/bluetooth/bm64/drv_bm64_gpio.h"
#include "system/tmr/sys_tmr.h"     // for SYS_TMR_HANDLE etc
...
SYS_TMR_HANDLE DRV_BM64_tmrHandle;
...
<#if CONFIG_INCLUDE_BM64_I2S == true>
...
<#if CONFIG_INCLUDE_DEPRECATED_MMI_COMMANDS == true>             
...
<#if CONFIG_INCLUDE_DEPRECATED_MMI_COMMANDS == false> 
...
void DRV_BM64_Timer_1ms(uintptr_t context, uint32_t currTick)
{ 
...
{        
    ...
    switch (gDrvBm64Obj.state) {
        case DRV_BM64_STATE_INITIALIZE_START:
            DRV_BM64_tmrHandle = SYS_TMR_CallbackPeriodic (1, 0, &DRV_BM64_Timer_1ms);
            if (DRV_BM64_tmrHandle == DRV_HANDLE_INVALID)
            {
#if defined( ENABLE_SYS_LOG )    
                SYS_LOG("----------------------------------------");
                SYS_LOG("- Invalid DRV_BM64_tmrHandle in drv_bm64.c");
                SYS_LOG("----------------------------------------");
#endif 
                while(1);
            }
        ...
        case DRV_BM64_STATE_POWER_ON:
            if(DRV_BM64_SystemStatus == DRV_BM64_SYSTEM_STANDBY)        //wait until status is standby
            {
                if(BM64_BLE_JUMPER_ENABLED)
                {
                    //BTAPP_timer1ms = 50;
