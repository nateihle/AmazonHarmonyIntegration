...
static GFX_Result prepareNextGlyph(GFXU_StringAssetReader* strReader)
{
    ...
    if(GFXU_DecodeCodePoint(strReader->table->encodingMode,
                            &strReader->stringAddress[strReader->stringIndex],
                            strReader->stringLength - strReader->stringIndex,
                            &glyph,
                            &strReader->glyphDataWidth) == GFX_FAILURE)
    {
        done(strReader);
    ...
    strReader->stringIndex += strReader->glyphDataWidth;
...
static GFX_Result drawGlyphRow(GFXU_StringAssetReader* strReader)
{
       ...
       (int32_t)strReader->glyphRow + strReader->y >= strReader->clipY + strReader->clipHeight)
    {
        ...
        if(strReader->stringIndex >= strReader->stringLength)
        {
            done(strReader);
        }
        else
