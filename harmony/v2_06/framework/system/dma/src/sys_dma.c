...
/*******************************************************************************
  ...
  Description:
    This function is used to maintain the DMA system service's internal state 
    machine. This function services events on a specific DMA channel.

  Remarks:
    This function is normally not called directly by an application.  It is
    called by the system's task/interrupt routines.
*/
void SYS_DMA_Tasks(SYS_MODULE_OBJ object, DMA_CHANNEL activeChannel)
{
    SYS_DMA_CHANNEL_OBJECT *chanObj;
    DMA_CHANNEL_INT_SOURCE chanIntSrc;
    SYS_DMA_TRANSFER_EVENT dmaEvent = SYS_DMA_TRANSFER_EVENT_NONE;
    ...
    if(true == PLIB_DMA_ChannelXINTSourceFlagGet( DMA_ID_0, activeChannel, DMA_INT_BLOCK_TRANSFER_COMPLETE))
    {
        ...
        chanObj->errorInfo = SYS_DMA_ERROR_NONE;
        dmaEvent = SYS_DMA_TRANSFER_EVENT_COMPLETE;
    }
    
    else if(true == PLIB_DMA_ChannelXINTSourceFlagGet( DMA_ID_0, activeChannel, DMA_INT_TRANSFER_ABORT))
    {
        ...
        /* Update error and event */
        chanObj->errorInfo = SYS_DMA_ERROR_NONE;
        dmaEvent = SYS_DMA_TRANSFER_EVENT_ABORT;
    }
    
    else if(true == PLIB_DMA_ChannelXINTSourceFlagGet(DMA_ID_0, activeChannel, DMA_INT_ADDRESS_ERROR))
    {
        /* Clear the Block transfer complete flag */
        PLIB_DMA_ChannelXINTSourceFlagClear(DMA_ID_0,activeChannel, DMA_INT_ADDRESS_ERROR);

        /* Update error and event */
        chanObj->errorInfo = SYS_DMA_ERROR_ADDRESS_ERROR;
        dmaEvent = SYS_DMA_TRANSFER_EVENT_ERROR;
    }
    ...
    if( (NULL != chanObj->pEventCallBack) && (SYS_DMA_TRANSFER_EVENT_NONE != dmaEvent) )
    {
        SYS_INT_SourceStatusClear(chanIntSrc);        
        chanObj->pEventCallBack(dmaEvent, (SYS_DMA_CHANNEL_HANDLE)chanObj, chanObj->hClientArg);
    }      
